...
internal class ZombieBadGuy : BadGuy
{
	...
	public ZombieBadGuy()
	{
		Init(Settings.Health, Settings.Damage, 40, 100, new Vector2(41f, 41f), 10, 50, "Zombie/Zombie/Zombie");
	...
	public override void Update()
	{
		...
		Vector2 vector = ZombieUtils.DefaultZombieGotoPosition;
		if (closestPlayer != -1)
		{
			vector = ZombieUtils.Players[closestPlayer].Position;
		}
		Vector2 vector2 = vector - _position;
		vector2.Normalize();
		Vector2 fromVector = GeometryHelper.AngleToV2(_rotation, 1f);
		fromVector.Normalize();
		float num = (float)GeometryHelper.GetSignedAngleBetween2DVectors(fromVector, vector2, vector2);
		if (num < 0.1f && num > -0.1f)
		{
			MoveTowardsPoint(vector, Settings.Speed);
		}
		else if (_body != null)
		{
			_body.LinearVelocity = new Vector2(0f, 0f);
			_rotation = GeometryHelper.TurnToFace(_position, vector, _rotation, Settings.TurnSpeed);
		}
		if (_body != null)
		{
