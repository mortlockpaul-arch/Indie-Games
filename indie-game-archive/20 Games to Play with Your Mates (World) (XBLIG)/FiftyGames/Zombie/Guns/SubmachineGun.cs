...
internal class SubmachineGun : Gun
{
	public static GunSettings Settings { get; set; }

	public SubmachineGun(ZombiePlayer owner)
		: base(owner)
	{
		...
		_gunSprite = ZombieUtils.ContentManager().Load<Texture2D>("Zombie/Guns/HoldSubMachineGun");
	}
	...
	public override List<Shot> Shoot(Vector2 origin, float mainRotation)
	{
		...
		if (_rounds > 0)
		{
			...
			for (int i = 0; i < _shotsAtOnce; i++)
			{
				...
				list.Add(item);
				ZombieUtils.PlaySound("Shoot Submachinegun");
			}
			_rounds--;
