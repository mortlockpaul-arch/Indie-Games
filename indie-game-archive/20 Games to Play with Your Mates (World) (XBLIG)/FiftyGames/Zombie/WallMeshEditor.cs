...
internal static class WallMeshEditor
{
	private static NavMesh _currentWallMesh;

	private static List<VertexPositionColor> _wallVerts;

	private static int _currentSelectedNodeID;

	private static bool _wallMeshChanged;

	private static Stack<MeshEditorHistory> _history;

	private static int _previousLinkNodeID;

	private static bool _isCreatingSelectionBox;

	private static Vector2 _selectionBoxStart;

	private static Vector2 _selectionBoxEnd;

	private static VertexPositionColor[] _selectionBoxVerts;

	private static List<int> _selectedNodes;

	private static Vector2 _startMouseDrag;

	private static List<Body> _wallBodies;

	public static void SetNavMesh(NavMesh newNavMesh)
	{
		...
		_currentWallMesh.LoadNavMesh(new StreamReader("Content/Zombie/Data/walls.wls").BaseStream);
		for (int i = 0; i < _currentWallMesh.LineMesh.Lines.Count; i++)
		{
			Body body = BodyFactory.CreateEdge(ZombieUtils.World(), ConvertUnits.ToSimUnits(_currentWallMesh.LineMesh.Lines[i].Start), ConvertUnits.ToSimUnits(_currentWallMesh.LineMesh.Lines[i].End));
		...
		list.Add(line2);
		Line line3 = new Line();
		line3.Start = new Vector2(1900f, 0f);
		line3.End = new Vector2(1900f, 1200f);
		list.Add(line3);
		Line line4 = new Line();
		line4.Start = new Vector2(0f, 1200f);
		line4.End = new Vector2(1900f, 1200f);
		list.Add(line4);
		for (int j = 0; j < list.Count; j++)
		{
			Body body2 = BodyFactory.CreateEdge(ZombieUtils.World(), ConvertUnits.ToSimUnits(list[j].Start), ConvertUnits.ToSimUnits(list[j].End));
			body2.BodyType = BodyType.Static;
			body2.Mass = 1000f;
			body2.UserData = _currentWallMesh.LineMesh.Lines[j];
			body2.Friction = 0f;
			_wallBodies.Add(body2);
		}
	}

	private static void RefreshWallMesh()
	{
		...
		{
			Body body = BodyFactory.CreateEdge(ZombieUtils.World(), _currentWallMesh.LineMesh.Lines[j].Start, _currentWallMesh.LineMesh.Lines[j].End);
			body.BodyType = BodyType.Static;
			body.Mass = 1000f;
			_wallBodies.Add(body);
		}
	...
	public static void Update(Vector2 offset, World physWorld, List<Body> _physWalls)
	{
		Vector2 vector = InputState.GetMouseCoords() - offset;
		...
		{
			int nearestNodeID = _currentWallMesh.LineMesh.GetNearestNodeID(vector, 15f);
			if (_selectedNodes.Count > 0)
			{
				if (nearestNodeID != -1)
				{
					_currentSelectedNodeID = nearestNodeID;
					_startMouseDrag = vector;
				}
				else
				{
					ResetSelection();
					_selectionBoxStart = vector;
				}
			}
			else if (nearestNodeID == -1 && !InputState.GetCurrentKeyboardState().IsKeyDown(Keys.LeftControl))
			{
				_currentWallMesh.LineMesh.AddNode(vector);
		...
		if (InputState.LeftButtonHeld())
		{
			if (_currentSelectedNodeID != -1)
			{
				if (_selectedNodes.Count > 0)
				{
					...
					{
						_currentWallMesh.LineMesh.MoveNode(_selectedNodes[i], _currentWallMesh.LineMesh.MeshNodes[_selectedNodes[i]]._position - (_startMouseDrag - vector));
					}
					_startMouseDrag = vector;
					_wallMeshChanged = true;
				}
				else
				{
					...
					_currentWallMesh.LineMesh.MoveNode(_currentSelectedNodeID, vector);
		...
		if (InputState.MouseStateChanged() && InputState.LeftButtonReleased())
		{
			RefreshWallMesh();
		}
		if (InputState.KeyboardStateChanged())
		{
			...
			{
				_currentWallMesh.SaveNavMesh(new StreamWriter("Content/Zombie/Data/walls.wls").BaseStream);
			}
		...
		{
			int nearestNodeID2 = _currentWallMesh.LineMesh.GetNearestNodeID(vector, 15f);
			if (nearestNodeID2 != -1)
			{
	...
	public static void Draw(SpriteBatch spriteBatch, Vector2 offset)
	{
		if (_currentWallMesh != null)
		{
			_currentWallMesh.Draw(spriteBatch, offset);
			_currentWallMesh.LineMesh.LineRenderer.DrawShape(_wallVerts.ToArray(), offset);
