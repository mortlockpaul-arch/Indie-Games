...
internal static class NavMeshEditor
{
	private static NavMesh _currentNavMesh;

	private static bool _navMeshChanged;

	private static int _currentSelectedNodeID;

	private static int _startOfLinkNodeID;

	private static int _startOfPathID;

	private static int _endOfPathID;

	private static List<VertexPositionColor> _pathVerts;

	public static List<int> ZombieSpawnersNodeList => _currentNavMesh.LineMesh.SpecialNodes;
	...
	public static void Update(Vector2 offset)
	{
		...
		{
			int nearestNodeID = _currentNavMesh.LineMesh.GetNearestNodeID(vector, 15f);
			...
			else
			{
				...
				if (nearestNodeID == -1)
				{
					_currentNavMesh.LineMesh.AddNode(vector);
					_navMeshChanged = true;
				}
				else
				{
		...
		if (InputState.LeftButtonHeld() && _currentSelectedNodeID != -1)
		{
			...
			_currentNavMesh.LineMesh.MoveNode(_currentSelectedNodeID, vector);
			_navMeshChanged = true;
		}
		...
		{
			int nearestNodeID2 = _currentNavMesh.LineMesh.GetNearestNodeID(vector, 15f);
			if (nearestNodeID2 != -1)
			{
		...
		{
			int nearestNodeID3 = _currentNavMesh.LineMesh.GetNearestNodeID(vector, 15f);
			if (nearestNodeID3 != -1)
			{
	...
	public static void Draw(SpriteBatch spriteBatch, Vector2 offset)
	{
		if (_currentNavMesh != null)
		{
			_currentNavMesh.Draw(spriteBatch, offset);
			_currentNavMesh.LineMesh.LineRenderer.DrawShape(_pathVerts.ToArray(), offset);
		}
	}
}
