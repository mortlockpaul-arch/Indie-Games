...
internal class WallMeshEditor
{
	...
	public void SetNavMesh(NavMesh newNavMesh)
	{
		...
		_currentWallMesh.LoadNavMesh(new StreamReader("Content/MicroMachines/Data/walls.wls").BaseStream);
	...
	public void Update(Vector2 mouseCoord, World physWorld)
	{
		...
		if (InputState.KeyboardStateChanged())
		{
			...
			{
				_currentWallMesh.SaveNavMesh(new StreamWriter("Content/MicroMachines/Data/walls.wls").BaseStream);
				_currentWallMesh.SaveNavMesh(new StreamWriter("Content/MicroMachines/Data/walls.wls").BaseStream);
			}
