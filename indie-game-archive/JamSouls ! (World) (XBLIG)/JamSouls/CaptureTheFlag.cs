...
internal class CaptureTheFlag : GameState
{
	...
	public override void Update(GameTime gameTime, bool otherScreenHasFocus, bool coveredByOtherScreen)
	{
		...
		if (!m_bTeamChoosen)
		{
			...
			m_RedFlag.m_Jam.UpdateFrame(gameTime.ElapsedGameTime.Milliseconds);
			m_BlueFlag.m_Jam.UpdateFrame(gameTime.ElapsedGameTime.Milliseconds);
		...
		else if (base.IsActive && !m_bIsPaused && !m_bGameOver)
		{
			...
			{
				if (player.m_Team == PlayerConfig.BLUE_TEAM_COLOR)
				{
					m_BlueTeamScore += player.m_Score;
				}
				else
				{
					m_RedTeamScore += player.m_Score;
				}
	...
	public void ManageChooseTeam()
	{
		...
		Vector2 Position = new Vector2(220f, tileSafeTop + 40);
		m_BluePotTex.Draw(new Vector2(0f, 0f), Color.White);
		m_BlueFlag.m_Jam.Draw(ref Position, SpriteEffects.None, Color.White, 1f);
		...
		Position = new Vector2(850f + base.ScreenManager.GoBoomBig.MeasureString(TextManager.GetText(TextID.REDTEAM)).X, tileSafeTop + 38);
		m_RedFlag.m_Jam.Draw(ref Position, SpriteEffects.None, Color.White, 1f);
		...
		if (!flag)
		{
			Vector2 Position2 = new Vector2(530f, 500 + tileSafeTop);
			m_btSpriteSoft[0].Draw(ref Position2, SpriteEffects.None, Color.White, 1f);
			Position2.X += m_btSpriteSoft[0].GetFrameWidth();
			Position2.Y += 20f;
			base.ScreenManager.SpriteBatch.DrawString(base.ScreenManager.GoBoomBig, TextManager.GetText(TextID.START_GAME), Position2, Color.White);
		}
	...
	public void DrawHud()
	{
		...
		if (m_ReadyTimer > 0f)
		{
			...
			m_GetReadyTex.Draw(readyPos, Color.White);
