...
public class MessageBoxScreen
{
	...
	public bool isBisYourOnlyEscape;
	...
	public MessageBoxScreen(Texture2D tex, float durationSeconds, bool isQTEa, bool isCutScenea, string sceneNamea, string uniqueNamea)
	{
		LoadContent();
		...
		if (isCutScene)
		{
			if (anywhereInput.B_pressed && (isSkippable || isBisYourOnlyEscape))
			{
				if (!isBisYourOnlyEscape)
				{
					...
					{
						TriggerManager.SetTriggerEvent(uniqueName + "SCENECANCELLED");
					}
			...
			if (anywhereInput.A_pressed && !isBisYourOnlyEscape)
			{
				...
				if (uniqueName != null && uniqueName != "")
				{
					TriggerManager.SetTriggerEvent(uniqueName + "SCENEOVER");
				}
				ExitScreen();
			}
		}
	...
	public void DrawForFlappy()
	{
		...
		if (message != "" && message != null)
		{
			...
			GraphicsManager.DrawRectangle(rectangle2, GraphicsManager.TheColorTransparentGray);
			GraphicsManager.DrawRectangle(rectangle2, GraphicsManager.TheColorTransparentGray);
			GraphicsManager.DrawString((int)vector2.X, (int)vector2.Y, message, Color.White, GraphicsManager.font);
		}
		if (texture != null)
		{
	...
	public void Draw()
	{
		...
		if (durationInSeconds == 0f)
		{
			...
			{
				string text = "okay";
				Vector2 vector3 = GraphicsManager.font.MeasureString(text);
				Rectangle rectangle3 = new Rectangle(backButtonRect.X, backButtonRect.Y, (int)vector3.X, (int)vector3.Y);
			...
			{
				GraphicsManager.Draw(GraphicsManager.imgNextButton, new Rectangle(0, 0, 1920, 1080), null, Color.White, 0f, Vector2.Zero, SpriteEffects.None, Definitions.LayerDepthTop);
			}
		}
		if (!isCutScene && !isQTE)
		{
