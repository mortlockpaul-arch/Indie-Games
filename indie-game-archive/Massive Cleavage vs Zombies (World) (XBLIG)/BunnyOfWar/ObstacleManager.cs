...
public static class ObstacleManager
{
	...
	public static void DrawObstacles()
	{
		...
		for (int i = 0; i < Obstacles.Count; i++)
		{
			...
			if (obstacleObject.name == null || !(obstacleObject.name != "") || !obstacleObject.isActive || (!obstacleObject.rect.Intersects(GraphicsManager.viewableArea) && obstacleObject.AImode != NonFighterAI.modes.FallingMeteoriteOneTime && obstacleObject.AImode != NonFighterAI.modes.FallingMeteorite))
			{
				continue;
			}
			flag = true;
			...
			float num2 = (float)(i + 1) * 1E-05f;
			GraphicsManager.Draw(GraphicsManager.LoadTexture("scenery/obstacles/" + obstacleObject.name), adjustedRectangle, null, Color.White, rotation, Vector2.Zero, effects, layerDepth + num2);
	...
	public static void ProcessObstacles()
	{
		for (int i = 0; i < Obstacles.Count; i++)
		{
			ObstacleObject obstacleObject = Obstacles[i].Copy();
			if (Obstacles[i].AImode != NonFighterAI.modes.none && Obstacles[i].isActive)
			{
				if (Obstacles[i].AImode == NonFighterAI.modes.MoveCircular || Obstacles[i].AImode == NonFighterAI.modes.MoveCircularSpeedup)
				{
					obstacleObject = NonFighterAI.moveCircular(obstacleObject);
				}
				else if (Obstacles[i].AImode == NonFighterAI.modes.MoveRocketWhenSatOn)
				{
					obstacleObject = NonFighterAI.MoveRocketWhenSatOn(obstacleObject);
				}
				else if (Obstacles[i].AImode == NonFighterAI.modes.SinkWhenSatOn)
				{
					obstacleObject = NonFighterAI.SinkWhenSatOn(obstacleObject);
				}
				else if (Obstacles[i].AImode == NonFighterAI.modes.RiseWhenSatOn)
				{
					obstacleObject = NonFighterAI.RiseWhenSatOn(obstacleObject);
				}
				else if (Obstacles[i].AImode == NonFighterAI.modes.MoveUpDown)
				{
					obstacleObject = NonFighterAI.moveUpDown(obstacleObject);
				}
				else if (Obstacles[i].AImode == NonFighterAI.modes.MoveLeftRight)
				{
					obstacleObject = NonFighterAI.moveLeftRight(obstacleObject);
				}
				else if (Obstacles[i].AImode == NonFighterAI.modes.MoveXWhenSatOn)
				{
					obstacleObject = NonFighterAI.moveXWhenSatOn(obstacleObject);
				}
				else if (Obstacles[i].AImode != NonFighterAI.modes.SpaceInvader)
				{
					if (Obstacles[i].AImode == NonFighterAI.modes.FallingMeteorite || Obstacles[i].AImode == NonFighterAI.modes.FallingMeteoriteOneTime)
					{
						obstacleObject = NonFighterAI.moveLikeaFallingMeteorite(Obstacles[i]);
					}
					else if (Obstacles[i].AImode == NonFighterAI.modes.RoadKill)
					{
						if (FighterManager.doCollisionCheck(obstacleObject.rect, humans: true, cupoos: false) != null)
						{
							obstacleObject.takeDamage(100, broadcast: true);
						}
					}
					else if (Obstacles[i].AImode == NonFighterAI.modes.Trampoline)
					{
						NonFighterAI.Trampoline(Obstacles[i]);
					}
				}
			}
			if (obstacleObject.isActive && (obstacleObject.xRoll != 0 || obstacleObject.yRoll != 0))
			{
				int num = (int)obstacleObject.X;
				obstacleObject.X += (float)obstacleObject.xRoll / (float)Definitions.UpdatesPerSecond;
				int num2 = (int)obstacleObject.Y;
				obstacleObject.Y += (float)obstacleObject.yRoll / (float)Definitions.UpdatesPerSecond;
				if (!obstacleObject.isReallyScenery)
				{
					FighterManager.doObstacleRollingDamage(obstacleObject);
					if (obstacleObject.X != (float)num || obstacleObject.Y != (float)num2)
					{
						FighterManager.MoveFighters(obstacleObject.rect, obstacleObject.X - (float)num, obstacleObject.Y - (float)num2);
					}
					if (doCollisionCheck(obstacleObject.rect, obstacleObject.rect, null, Obstacles[i], null).HasValue)
					{
						obstacleObject.xRoll = 0;
						obstacleObject.yRoll = 0;
					}
				}
				if (!Definitions.ScreenMaxRect.Contains(obstacleObject.rect))
				{
					obstacleObject.xRoll = 0;
					obstacleObject.yRoll = 0;
				}
			}
			if (obstacleObject.isBeingCarriedBy != null)
			{
				if (obstacleObject.isBeingCarriedBy.PROPERTIES.holdingObstacleObject == null)
				{
					obstacleObject.isBeingCarriedBy = null;
				}
				else
				{
					obstacleObject.X = obstacleObject.isBeingCarriedBy.X - obstacleObject.width / 2 + obstacleObject.isBeingCarriedBy.width / 2;
					obstacleObject.Y = obstacleObject.isBeingCarriedBy.Y - obstacleObject.height - obstacleObject.isBeingCarriedBy.JUMP.jumpPixelsOffGround;
				}
			}
			if (obstacleObject.pixelsInTheAir > 0 || obstacleObject.isFalling)
			{
				obstacleObject.pixelsInTheAir -= obstacleObject.fallSpeedPerFrame;
				if (obstacleObject.pixelsInTheAir <= 0)
				{
					if (!SceneryManager.AmIOnSolidGround(obstacleObject.rect, obstacleObject) && !doCollisionCheck(obstacleObject.rect, obstacleObject.rect, null, Obstacles[i], null).HasValue)
					{
						obstacleObject.Y += obstacleObject.fallSpeedPerFrame;
						obstacleObject.pixelsInTheAir = 1;
					}
					else
					{
			...
			Obstacles[i] = obstacleObject.Copy();
		}
	}
