...
public static class TriggerManager
{
	...
	public static void AddTimeFromPause()
	{
		...
		for (int i = 0; i < triggers.Count; i++)
		{
			if (triggers[i].activationTime != DateTime.MaxValue)
			{
				triggers[i].activationTime = triggers[i].activationTime.AddMilliseconds(1000f / (float)Definitions.UpdatesPerSecond);
			}
		}
	}
	...
	public static void BroadcastTriggerTriggered(int id, int x, int y)
	{
		if (Networking.NullCheckSucceed())
		{
			...
			packetWriter.Write((ushort)id);
			packetWriter.Write((ushort)x);
			packetWriter.Write((ushort)y);
			Networking.networkSession.LocalGamers[0].SendData(packetWriter, SendDataOptions.Reliable);
		}
	}
