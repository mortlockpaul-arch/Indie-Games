...
public static class FighterManager
{
	...
	private static int totalX = 5;

	private static int nextY = 0;

	private static int totalY = 3;
	...
	public static void ProcessFighters()
	{
		...
		{
			if (!computerPlayers[i].PROPERTIES.isFlying && !computerPlayers[i].JUMP.jumperIsOnWayUp && !SceneryManager.AmIOnSolidGround(computerPlayers[i].getWhereFeetAre(), null))
			{
				computerPlayers[i].fallDownBecauseYouFellOffACliff();
			}
			computerPlayers[i].JUMP.ProcessJumpStuff();
			if (computerPlayers[i].PROPERTIES.isAlive)
			{
				int num3 = SceneryManager.AmITakingSceneryDPS(computerPlayers[i].getWhereBodyIs(), computerPlayers[i].getWhereFeetAre(), computerPlayers[i].PROPERTIES.isCrouching, computerPlayers[i].PROPERTIES.isKicking);
				if (num3 > 0)
				{
					computerPlayers[i].healthChange((float)(-num3) / (float)Definitions.UpdatesPerSecond);
				}
			}
			if (computerPlayers[i].PROPERTIES.momentum != Vector2.Zero)
			{
				computerPlayers[i].move(computerPlayers[i].PROPERTIES.momentum.X, computerPlayers[i].PROPERTIES.momentum.Y);
				computerPlayers[i].PROPERTIES.momentum.X = computerPlayers[i].PROPERTIES.momentum.X / 2f;
				computerPlayers[i].PROPERTIES.momentum.Y = computerPlayers[i].PROPERTIES.momentum.Y / 2f;
				if (computerPlayers[i].PROPERTIES.momentum.X < 10f && computerPlayers[i].PROPERTIES.momentum.Y < 10f)
				{
					computerPlayers[i].PROPERTIES.momentum = Vector2.Zero;
				}
			}
			if (computerPlayers[i].PROPERTIES.carriedByFighter != null)
			{
				computerPlayers[i].X = computerPlayers[i].PROPERTIES.carriedByFighter.X - computerPlayers[i].width / 2 + computerPlayers[i].PROPERTIES.carriedByFighter.width / 2;
				computerPlayers[i].Y = computerPlayers[i].PROPERTIES.carriedByFighter.Y - computerPlayers[i].height - computerPlayers[i].PROPERTIES.carriedByFighter.JUMP.jumpPixelsOffGround;
			}
		}
	}
	...
	public static FighterObject getHumanPlayerWhoseMostRightForNetworking()
	{
		...
		{
			if (list[j].X > fighterObject.X)
			{
				fighterObject = list[j];
			}
		}
		return fighterObject;
	}
	...
	public static void AddComputerPlayer(FighterObject fo)
	{
		...
		{
			computerPlayers = new List<FighterObject>(50);
		}
		fo.ID = computerPlayers.Count;
	...
	public static List<FighterObject> getComputerPlayersAliveOrBleeding()
	{
		List<FighterObject> list = new List<FighterObject>(computerPlayers.Count);
		foreach (FighterObject computerPlayer in computerPlayers)
		{
			if (computerPlayer.PROPERTIES.isAlive || computerPlayer.PROPERTIES.isBleeding)
			{
				list.Add(computerPlayer);
			}
		}
	...
	public static List<FighterObject> getComputerPlayersWhoDead()
	{
		List<FighterObject> list = new List<FighterObject>(computerPlayers.Count);
		foreach (FighterObject computerPlayer in computerPlayers)
		{
			if (!computerPlayer.PROPERTIES.isAlive)
			{
				list.Add(computerPlayer);
			}
		}
	...
	public static int doTrampolineBounce(Rectangle r, bool hitHumans, bool hitCPUs)
	{
		int num = 0;
		if (hitHumans)
		{
			foreach (FighterObject humanPlayer in humanPlayers)
			{
				if (humanPlayer.PROPERTIES.CpuAttackCooldown < DateTime.Now && humanPlayer.PROPERTIES.isAlive && r.Intersects(humanPlayer.getWhereBodyIs()))
				{
					num++;
					if (humanPlayer.PROPERTIES.circlePivotPoint == Vector2.Zero)
					{
						humanPlayer.PROPERTIES.circlePivotPoint.Y = -200f;
						humanPlayer.PROPERTIES.velocity.Y = -200f;
					}
					else
					{
						humanPlayer.PROPERTIES.circlePivotPoint.Y = humanPlayer.PROPERTIES.circlePivotPoint.Y * 1.25f;
						humanPlayer.PROPERTIES.velocity.Y = humanPlayer.PROPERTIES.circlePivotPoint.Y;
					}
					humanPlayer.PROPERTIES.CpuAttackCooldown = DateTime.Now.AddSeconds(2.0);
				}
			}
		}
		return num;
	...
	public static int doObstacleRollingDamage(ObstacleObject o)
	{
		...
		for (int j = 0; j < computerPlayers.Count; j++)
		{
			if (computerPlayers[j].PROPERTIES.isAlive && rect.Intersects(computerPlayers[j].getWhereBodyIs()))
			{
				num++;
				computerPlayers[j].hitMeWithObstacle(o);
			}
		}
		return num;
	}
	...
	{
		CustomsManager.GetCustomPlayerAnimation();
		new FighterObject();
		for (int i = 0; i < humanPlayers.Count; i++)
		{
	...
	public static FighterObject remakeHumanAnimations(FighterObject fighter)
	{
		string customPlayerAnimation = CustomsManager.GetCustomPlayerAnimation();
		string text = Definitions.ContentRootDirectory + "/fighters/hero";
		float scale = fighter.PROPERTIES.scale;
		if (customPlayerAnimation == "")
		{
			float num = 1f;
			fighter.animationQuickPunching = new Animation(frameTime: (!RandomStaticGlobals.isHardMode) ? 0.04f : 0.02f, animationFileNames: new string[5]
			{
				text + "/attackX1.png",
				text + "/attackX1.png",
				text + "/attackX1.png",
				text + "/attackX4.png",
				text + "/attackX3.png"
			}, isLooping: false, scale: scale);
			fighter.animationPunching = new Animation(frameTime: (!RandomStaticGlobals.isHardMode) ? 0.1f : 0.05f, animationFileNames: new string[3]
			{
				text + "/attack1.png",
				text + "/attack2.png",
				text + "/attack3.png"
	...
	public static FighterObject makeNewHumanPlayer(PlayerIndex? playerIndex, bool isNetworkPlayer, string gamerTag, float scale)
	{
		...
		if (customPlayerAnimation == "")
		{
			fighterObject = createNewHuman(RandomStaticGlobals.Content, GraphicsManager.BoundariesDefault, 100, 800, isAlive: true, BunnyOfWar.AI.AI.modes.doNothing, scale);
		}
	...
	public static FighterObject createNewHuman(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale)
	{
		FighterObject fighterObject = new FighterObject(Content);
		fighterObject.setCollisionRects(boundaries, new Rectangle(210, 0, 92, 512), new Rectangle(50, 0, 412, 512), new Rectangle(190, 400, 140, 112));
		fighterObject.PROPERTIES.moveSpeed = Definitions.MoveSpeedHuman;
		...
		string text = Definitions.ContentRootDirectory + "/fighters/hero";
		fighterObject.PROPERTIES.directoryName = text;
		fighterObject.animationIdle = new Animation(new string[1] { text + "/idle.png" }, 0.1f, isLooping: true, scale);
		fighterObject = remakeHumanAnimations(fighterObject);
		fighterObject.animationWalking = new Animation(new string[10]
		{
			text + "/walking02.png",
			text + "/walking05.png",
			text + "/walking08.png",
			...
			text + "/walking26.png",
			text + "/walking29.png"
		}, 0.1f, isLooping: true, scale);
		fighterObject.animationDying = new Animation(text + "/idle.png", 1f, isLooping: true, scale);
		fighterObject.animationPooingStart = new Animation(text + "/idle.png", 0.5f, isLooping: true, scale);
		fighterObject.animationPooingFinished = new Animation(text + "/idle.png", 1.5f, isLooping: true, scale);
	...
	public static FighterObject createNewEnemyXsuitlady(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale)
	{
		int num = 10;
		int num2 = 5;
		int moveSpeed = 4;
		FighterObject fighterObject = new FighterObject(Content);
		fighterObject.setCollisionRects(boundaries, new Rectangle(150, 0, 362, 512), new Rectangle(210, 0, 120, 512), new Rectangle(190, 400, 140, 112));
		fighterObject.PROPERTIES.isAlive = isAlive;
		fighterObject.PROPERTIES.AImode = aiMode;
		...
		fighterObject.PROPERTIES.DamageFromAttack = num;
		fighterObject.PROPERTIES.DamageFromQuickAttack = num2;
		string text = Definitions.ContentRootDirectory + "/fighters/Xsuitlady";
		fighterObject.PROPERTIES.directoryName = text;
	...
	public static FighterObject createNewEnemyXcasualman(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale)
	{
		int num = 10;
		int num2 = 5;
		int moveSpeed = 5;
		FighterObject fighterObject = new FighterObject(Content);
		fighterObject.setCollisionRects(boundaries, new Rectangle(150, 0, 362, 512), new Rectangle(200, 0, 130, 512), new Rectangle(190, 400, 140, 112));
		fighterObject.PROPERTIES.isAlive = isAlive;
		fighterObject.PROPERTIES.AImode = aiMode;
		fighterObject.RANGED.rangedProjectileType = ProjectileManager.ProjectileType.bullet;
		fighterObject.RANGED.rangedDamage = 5;
		...
		fighterObject.PROPERTIES.DamageFromAttack = num;
		fighterObject.PROPERTIES.DamageFromQuickAttack = num2;
		string text = Definitions.ContentRootDirectory + "/fighters/Xcasualman";
		fighterObject.PROPERTIES.directoryName = text;
		fighterObject.animationIdle = new Animation(new string[1] { text + "/idle.png" }, 0.8f, isLooping: true, scale);
		fighterObject.animationQuickPunching = new Animation(new string[3]
		{
		...
		fighterObject.animationWalking = new Animation(new string[3]
		{
			text + "/walk1.png",
			text + "/walk2.png",
			text + "/walk3.png"
	...
	public static FighterObject createNewEnemyXsuitman(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale)
	{
		int num = 10;
		int num2 = 5;
		int moveSpeed = 4;
		FighterObject fighterObject = new FighterObject(Content);
		fighterObject.setCollisionRects(boundaries, new Rectangle(150, 0, 362, 512), new Rectangle(220, 0, 110, 512), new Rectangle(190, 400, 140, 112));
		fighterObject.PROPERTIES.isAlive = isAlive;
		fighterObject.PROPERTIES.AImode = aiMode;
		fighterObject.RANGED.rangedProjectileType = ProjectileManager.ProjectileType.bullet;
		fighterObject.RANGED.rangedDamage = 5;
		...
		fighterObject.PROPERTIES.DamageFromAttack = num;
		fighterObject.PROPERTIES.DamageFromQuickAttack = num2;
		string text = Definitions.ContentRootDirectory + "/fighters/Xsuitman";
		fighterObject.PROPERTIES.directoryName = text;
		fighterObject.animationIdle = new Animation(new string[1] { text + "/idle.png" }, 0.8f, isLooping: true, scale);
		fighterObject.animationQuickPunching = new Animation(new string[3]
		{
		...
		fighterObject.animationWalking = new Animation(new string[2]
		{
			text + "/walk4.png",
			text + "/walk5.png"
		}, 0.225f, isLooping: true, scale);
	...
	public static FighterObject createNewEnemyXskinless(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale)
	{
		int num = 12;
		int num2 = 5;
		int moveSpeed = 3;
		FighterObject fighterObject = new FighterObject(Content);
		fighterObject.setCollisionRects(boundaries, new Rectangle(150, 0, 362, 512), new Rectangle(190, 0, 140, 512), new Rectangle(190, 400, 140, 112));
		fighterObject.PROPERTIES.isAlive = isAlive;
		fighterObject.PROPERTIES.AImode = aiMode;
		fighterObject.RANGED.rangedProjectileType = ProjectileManager.ProjectileType.bullet;
		fighterObject.RANGED.rangedDamage = 5;
		...
		fighterObject.PROPERTIES.DamageFromAttack = num;
		fighterObject.PROPERTIES.DamageFromQuickAttack = num2;
		string text = Definitions.ContentRootDirectory + "/fighters/Xskinless";
		fighterObject.PROPERTIES.directoryName = text;
		fighterObject.animationIdle = new Animation(new string[1] { text + "/idle.png" }, 0.8f, isLooping: true, scale);
		fighterObject.animationQuickPunching = new Animation(new string[3]
		{
	...
	public static FighterObject createNewEnemyX(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale, int dogsToo)
	{
		nextX++;
		if (nextX >= totalX)
		{
		...
		if (nextX == 0)
		{
			fighterObject = createNewEnemyXdogwhite(Content, boundaries, X, Y - 50, isAlive, aiMode, scale);
		}
		if (nextX == 1)
		{
			fighterObject = createNewEnemyXsuitlady(Content, boundaries, X, Y, isAlive, aiMode, scale);
		}
		if (nextX == 2)
		{
			fighterObject = createNewEnemyXcasualman(Content, boundaries, X, Y, isAlive, aiMode, scale);
		}
		if (nextX == 3)
		{
			fighterObject = createNewEnemyXskinless(Content, boundaries, X, Y, isAlive, aiMode, scale);
		}
		if (nextX == 4)
		{
			fighterObject = createNewEnemyXsuitman(Content, boundaries, X, Y, isAlive, aiMode, scale);
		}
		if (aiMode == BunnyOfWar.AI.AI.modes.Xweak)
		{
			fighterObject.PROPERTIES.AImode = BunnyOfWar.AI.AI.modes.X;
			fighterObject.PROPERTIES.DamageFromAttack = 1f;
		}
		else
		{
			fighterObject.PROPERTIES.DamageFromAttack = 10f;
		}
		return fighterObject;
	}

	public static FighterObject createNewEnemyY(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale, int dogsToo)
	{
		nextY++;
		if (nextY >= totalY)
		{
		...
		if (nextY == 0)
		{
			fighterObject = createNewEnemyXYdogmean(Content, boundaries, X, Y + 170, isAlive, aiMode, 0.8f);
		}
		if (nextY == 1)
		{
			fighterObject = createNewEnemyYboylowres(Content, boundaries, X, Y, isAlive, aiMode, scale);
		}
		if (nextY == 2)
		{
			fighterObject = createNewEnemyYskinlesscrawler(Content, boundaries, X, Y, isAlive, aiMode, scale);
		}
		if (aiMode == BunnyOfWar.AI.AI.modes.Yweak)
		{
			fighterObject.PROPERTIES.AImode = BunnyOfWar.AI.AI.modes.Y;
			fighterObject.PROPERTIES.DamageFromAttack = 1f;
		}
		else
		{
	...
	public static FighterObject createNewEnemyYskinlesscrawler(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale)
	{
		int num = 10;
		int num2 = 5;
		int moveSpeed = 2;
		FighterObject fighterObject = new FighterObject(Content);
		fighterObject.setCollisionRects(boundaries, new Rectangle(90, 200, 430, 312), new Rectangle(150, 0, 212, 512), new Rectangle(190, 400, 140, 112));
		fighterObject.PROPERTIES.isAlive = isAlive;
		fighterObject.PROPERTIES.AImode = aiMode;
		fighterObject.RANGED.rangedProjectileType = ProjectileManager.ProjectileType.bullet;
		fighterObject.RANGED.rangedDamage = 5;
		...
		fighterObject.PROPERTIES.DamageFromAttack = num;
		fighterObject.PROPERTIES.DamageFromQuickAttack = num2;
		string text = Definitions.ContentRootDirectory + "/fighters/Yskinlesscrawler";
		fighterObject.PROPERTIES.directoryName = text;
		fighterObject.animationIdle = new Animation(new string[1] { text + "/idle.png" }, 0.8f, isLooping: true, scale);
		...
		fighterObject.animationWalking = new Animation(new string[5]
		{
			text + "/walk1.png",
			text + "/walk2.png",
			text + "/walk3.png",
	...
	public static FighterObject createNewEnemyYboylowres(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale)
	{
		int num = 10;
		int num2 = 5;
		int moveSpeed = 3;
		FighterObject fighterObject = new FighterObject(Content);
		fighterObject.setCollisionRects(boundaries, new Rectangle(150, 200, 362, 312), new Rectangle(190, 0, 140, 512), new Rectangle(190, 300, 140, 212));
		fighterObject.PROPERTIES.isAlive = isAlive;
		fighterObject.PROPERTIES.AImode = aiMode;
		fighterObject.RANGED.rangedProjectileType = ProjectileManager.ProjectileType.bullet;
		fighterObject.RANGED.rangedDamage = 5;
		...
		fighterObject.PROPERTIES.DamageFromAttack = num;
		fighterObject.PROPERTIES.DamageFromQuickAttack = num2;
		string text = Definitions.ContentRootDirectory + "/fighters/Yboylowres";
		fighterObject.PROPERTIES.directoryName = text;
		fighterObject.animationIdle = new Animation(new string[1] { text + "/idle.png" }, 0.8f, isLooping: true, scale);
		fighterObject.animationQuickPunching = new Animation(new string[3]
		{
	...
	public static FighterObject createNewEnemyXYdogmean(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale)
	{
		int num = 10;
		int num2 = 5;
		int moveSpeed = 12;
		FighterObject fighterObject = new FighterObject(Content);
		fighterObject.PROPERTIES.name = "dog";
		fighterObject.setCollisionRects(boundaries, new Rectangle(100, 200, 312, 312), new Rectangle(100, 0, 312, 512), new Rectangle(190, 400, 140, 112));
		fighterObject.PROPERTIES.isAlive = isAlive;
		fighterObject.PROPERTIES.AImode = aiMode;
		fighterObject.RANGED.rangedProjectileType = ProjectileManager.ProjectileType.bullet;
		fighterObject.RANGED.rangedDamage = 5;
		...
		fighterObject.PROPERTIES.DamageFromAttack = num;
		fighterObject.PROPERTIES.DamageFromQuickAttack = num2;
		string text = Definitions.ContentRootDirectory + "/fighters/XYdogmean";
		fighterObject.PROPERTIES.directoryName = text;
		fighterObject.animationIdle = new Animation(new string[1] { text + "/idle.png" }, 0.8f, isLooping: true, scale);
		fighterObject.animationQuickPunching = new Animation(new string[3]
		{
	...
	public static FighterObject createNewEnemyXdogwhite(ContentManager Content, Rectangle boundaries, int X, int Y, bool isAlive, BunnyOfWar.AI.AI.modes aiMode, float scale)
	{
		int num = 10;
		int num2 = 5;
		int moveSpeed = 10;
		FighterObject fighterObject = new FighterObject(Content);
		fighterObject.PROPERTIES.name = "dog";
		fighterObject.setCollisionRects(boundaries, new Rectangle(100, 200, 312, 312), new Rectangle(100, 100, 312, 256), new Rectangle(190, 400, 140, 112));
		fighterObject.PROPERTIES.isAlive = isAlive;
		fighterObject.PROPERTIES.AImode = aiMode;
		fighterObject.RANGED.rangedProjectileType = ProjectileManager.ProjectileType.bullet;
		fighterObject.RANGED.rangedDamage = 5;
		...
		fighterObject.PROPERTIES.DamageFromAttack = num;
		fighterObject.PROPERTIES.DamageFromQuickAttack = num2;
		string text = Definitions.ContentRootDirectory + "/fighters/Xdogwhite";
		fighterObject.PROPERTIES.directoryName = text;
		fighterObject.animationIdle = new Animation(new string[1] { text + "/walk1.png" }, 0.8f, isLooping: true, scale);
		fighterObject.animationQuickPunching = new Animation(new string[5]
		{
			...
			text + "/attack4.png",
			text + "/attack5.png"
		}, 0.1f, isLooping: false, scale);
		fighterObject.animationPunching = new Animation(new string[5]
		{
			text + "/attack1.png",
			text + "/attack2.png",
			text + "/attack3.png",
	...
	public static FighterObject MakeNewFighterObjectFromName(string name, FighterObject fo)
	{
		...
		{
		case "X":
			fighterObject = createNewEnemyX(LevelManager.Content, GraphicsManager.BoundariesDefault, fo.X, fo.Y, fo.PROPERTIES.isAlive, fo.PROPERTIES.AImode, 1f, 0);
			break;
		case "Y":
			fighterObject = createNewEnemyY(LevelManager.Content, GraphicsManager.BoundariesDefault, fo.X, fo.Y, fo.PROPERTIES.isAlive, fo.PROPERTIES.AImode, 1.3f, 0);
			break;
		case "Xweak":
			fighterObject = createNewEnemyX(LevelManager.Content, GraphicsManager.BoundariesDefault, fo.X, fo.Y, fo.PROPERTIES.isAlive, fo.PROPERTIES.AImode, 1f, 0);
			break;
		case "Yweak":
			...
			break;
		case "Xdog":
			fighterObject = createNewEnemyXdogwhite(LevelManager.Content, GraphicsManager.BoundariesDefault, fo.X, fo.Y, fo.PROPERTIES.isAlive, fo.PROPERTIES.AImode, 1f);
			break;
		case "Ydog":
			fighterObject = createNewEnemyXYdogmean(LevelManager.Content, GraphicsManager.BoundariesDefault, fo.X, fo.Y, fo.PROPERTIES.isAlive, fo.PROPERTIES.AImode, 1.3f);
			break;
		}
	...
	public static void adjustScreenViewableAreaForThisPlayer(FighterObject fighter, ref Rectangle viewableArea)
	{
		...
		if (RandomStaticGlobals.GameMode == Definitions.GameMode.zelda || RandomStaticGlobals.GameMode == Definitions.GameMode.shooter || RandomStaticGlobals.GameMode == Definitions.GameMode.space || RandomStaticGlobals.GameMode == Definitions.GameMode.gunsmoke || RandomStaticGlobals.GameMode == Definitions.GameMode.trampoline)
		{
	...
	public static void ResetHumanPlayers()
	{
		...
		for (int i = 0; i < humanPlayers.Count; i++)
		{
			...
			RandomStaticGlobals.imgFailBackground = null;
		}
	}
	...
	public static void ResetHumanPlayersXY()
	{
		for (int i = 0; i < humanPlayers.Count; i++)
		{
			...
			{
				humanPlayers[i].X = 1000 - i * 50;
				humanPlayers[i].Y = 999500 - i * 20;
			}
			else if (RandomStaticGlobals.GameMode == Definitions.GameMode.brawlerX)
			{
				humanPlayers[i].X = 1000 - i * 100;
				humanPlayers[i].Y = 400;
			}
		}
	}
	...
	public static void BroadcastAnimationChange(int ID, FighterObjectProperties.AnimationName animation, Definitions.facing facing)
	{
		...
		{
			Networking.packetWriter.Write((byte)3);
			Networking.packetWriter.Write((byte)ID);
			Networking.packetWriter.Write((byte)animation);
			Networking.packetWriter.Write((byte)facing);
			Networking.networkSession.LocalGamers[0].SendData(Networking.packetWriter, SendDataOptions.None);
		}
	}

	public static void ReadAnimationChange(PacketReader pr)
	{
		int index = pr.ReadByte();
		byte animation = pr.ReadByte();
		byte isFacing = pr.ReadByte();
		try
		{
	...
	public static void BroadcastHumanHealth(int playerID, int health)
	{
		if (Networking.NullCheckSucceed())
		{
			Networking.packetWriter.Write((byte)4);
			Networking.packetWriter.Write((byte)playerID);
			Networking.packetWriter.Write((byte)health);
			Networking.networkSession.LocalGamers[0].SendData(Networking.packetWriter, SendDataOptions.None);
		}
	}
	...
	public static void BroadcastFighterStunned(int playerID, bool areWeHuman, double durationInSeconds)
	{
		if (Networking.NullCheckSucceed())
		{
			Networking.packetWriter.Write((byte)21);
			Networking.packetWriter.Write((ushort)playerID);
			Networking.packetWriter.Write(areWeHuman);
			Networking.packetWriter.Write(durationInSeconds);
			Networking.networkSession.LocalGamers[0].SendData(Networking.packetWriter, SendDataOptions.None);
		}
	}
	...
	public static void BroadcastAddProjectile(int playerID, int x, int y, Vector2 direction, float speed, bool areWeHuman, int ProjectileTypeWhatsIsIt, int width, int height, int damage)
	{
		if (Networking.NullCheckSucceed())
		{
			Networking.packetWriter.Write((byte)7);
			Networking.packetWriter.Write((byte)playerID);
			Networking.packetWriter.Write((uint)x);
			Networking.packetWriter.Write((uint)y);
			Networking.packetWriter.Write(direction);
			Networking.packetWriter.Write((double)speed);
			Networking.packetWriter.Write(areWeHuman);
			Networking.packetWriter.Write((ushort)ProjectileTypeWhatsIsIt);
			Networking.packetWriter.Write((uint)width);
			Networking.packetWriter.Write((uint)height);
			Networking.packetWriter.Write((uint)damage);
			Networking.networkSession.LocalGamers[0].SendData(Networking.packetWriter, SendDataOptions.None);
		}
	}
	...
	public static void ReadAddProjectile(PacketReader pr)
	{
		...
		{
			int index = pr.ReadByte();
			int num = (int)pr.ReadUInt32();
			int num2 = (int)pr.ReadUInt32();
			Vector2 direction = pr.ReadVector2();
			float speed = (float)pr.ReadDouble();
			bool flag = pr.ReadBoolean();
			int whatsIsIt = pr.ReadUInt16();
			int width = (int)pr.ReadUInt32();
			int height = (int)pr.ReadUInt32();
			int damage = (int)pr.ReadUInt32();
			if (flag)
			{
	...
	public static void BroadcastRangedAttack(int playerID, float x, float y)
	{
		if (Networking.NullCheckSucceed())
		{
			Networking.packetWriter.Write((byte)6);
			Networking.packetWriter.Write((byte)playerID);
			Networking.packetWriter.Write((double)x);
			Networking.packetWriter.Write((double)y);
			Networking.networkSession.LocalGamers[0].SendData(Networking.packetWriter, SendDataOptions.None);
		}
	}
	...
	public static void BroadcastFighterDeath(int ID, bool areWeHuman)
	{
		if (Networking.NullCheckSucceed())
		{
			Networking.packetWriter.Write((byte)19);
			Networking.packetWriter.Write((ushort)ID);
			Networking.packetWriter.Write(areWeHuman);
			Networking.networkSession.LocalGamers[0].SendData(Networking.packetWriter, SendDataOptions.Reliable);
		}
	}

	public static void ReadFighterDeath(PacketReader pr)
	{
		int iD = pr.ReadUInt16();
		bool human = pr.ReadBoolean();
		try
		{
	...
	public static void KillFighter(int ID, bool human)
	{
		...
		for (int j = 0; j < humanPlayers.Count; j++)
		{
			if (humanPlayers[j].ID == ID)
			{
				humanPlayers[j].PROPERTIES.health = 0f;
				humanPlayers[j].PROPERTIES.isAlive = false;
				humanPlayers[j].PROPERTIES.isDying = false;
			}
		}
	}
	...
	public static void BroadcastComputerHealthChange(int ID, int healthChangeAmount)
	{
		if (Networking.NullCheckSucceed())
		{
			Networking.packetWriter.Write((byte)5);
			Networking.packetWriter.Write((ushort)ID);
			Networking.packetWriter.Write((short)healthChangeAmount);
			Networking.networkSession.LocalGamers[0].SendData(Networking.packetWriter, SendDataOptions.Reliable);
		}
	}
	...
	{
		int num = pr.ReadUInt16();
		int num2 = pr.ReadInt16();
		try
		{
			for (int i = 0; i < computerPlayers.Count; i++)
			{
