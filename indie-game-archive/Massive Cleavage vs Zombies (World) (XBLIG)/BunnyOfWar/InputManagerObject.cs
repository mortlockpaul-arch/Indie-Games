...
public class InputManagerObject
{
	...
	public static int handleQTEInput(InputFromAnywhere anywhereInput, FighterObject player)
	{
		int num = 0;
		if (QuickTimeEventsManager.hasQTE)
		{
	...
	public void processCutsceneOrQTEInput(FighterObject player)
	{
		...
		if (QuickTimeEventsManager.hasQTE)
		{
			num = handleQTEInput(playerInput, player);
		}
		else
		{
			handleMessagesInput(playerInput, player);
		...
		if (num > 0 || player.PROPERTIES.health <= 0f || (QuickTimeEventsManager.hasQTE && QuickTimeEventsManager.QTE.expires < DateTime.Now))
		{
			player.onDeath();
			QuickTimeEventsManager.FAIL(player);
	...
	public void processBrawlerXInput(FighterObject player, List<FighterObject> enemies)
	{
		if (!player.PROPERTIES.areWeHuman)
		{
			BunnyOfWar.AI.AI.doSomething(player, enemies);
		...
		player.PROPERTIES.isWalking = false;
		if (player.PROPERTIES.Attack1After == DateTime.MinValue && player.PROPERTIES.Attack2After == DateTime.MinValue && (playerInput.LEFT_pressed || playerInput.LEFT_held))
		{
			player.moveLeft();
		}
		if (player.PROPERTIES.Attack1After == DateTime.MinValue && player.PROPERTIES.Attack2After == DateTime.MinValue && (playerInput.RIGHT_pressed || playerInput.RIGHT_held))
		{
			player.moveRight();
		}
		player.onIdle();
		if ((playerInput.X_pressed || playerInput.KB_A_pressed) && player.PROPERTIES.Attack1After == DateTime.MinValue && player.PROPERTIES.Attack2After == DateTime.MinValue)
		{
			player.PROPERTIES.ChangeAnimationAfter = DateTime.MinValue;
			player.onIdle();
			player.PlayAnimation(FighterObjectProperties.AnimationName.QuickPunching, broadcastThis: true);
			if (!RandomStaticGlobals.isHardMode)
			{
				player.PROPERTIES.Attack1After = DateTime.Now.AddMilliseconds(200.0);
			}
			else
			{
				player.PROPERTIES.Attack1After = DateTime.Now.AddMilliseconds(100.0);
			}
			player.PROPERTIES.ChangeAnimationAfter = DateTime.Now.AddMilliseconds(500.0);
		}
		if (!playerInput.B_pressed)
		{
			_ = playerInput.D_pressed;
		}
		if ((playerInput.Y_pressed || playerInput.W_pressed) && player.PROPERTIES.Attack1After == DateTime.MinValue && player.PROPERTIES.Attack2After == DateTime.MinValue)
		{
			player.PROPERTIES.ChangeAnimationAfter = DateTime.MinValue;
			player.onIdle();
			player.PlayAnimation(FighterObjectProperties.AnimationName.Punching, broadcastThis: true);
			if (!RandomStaticGlobals.isHardMode)
			{
				player.PROPERTIES.Attack2After = DateTime.Now.AddMilliseconds(200.0);
			}
			else
			{
				player.PROPERTIES.Attack2After = DateTime.Now.AddMilliseconds(100.0);
			}
			player.PROPERTIES.ChangeAnimationAfter = DateTime.Now.AddMilliseconds(500.0);
		}
		FighterObject fighterObject = null;
		int num = 33000;
		if (player.PROPERTIES.Attack1After != DateTime.MinValue && player.PROPERTIES.Attack1After < DateTime.Now)
		{
			player.PROPERTIES.Attack1After = DateTime.MinValue;
			for (int i = 0; i < FighterManager.computerPlayers.Count; i++)
			{
				if (!FighterManager.computerPlayers[i].PROPERTIES.isAlive || FighterManager.computerPlayers[i].PROPERTIES.AImode != BunnyOfWar.AI.AI.modes.X || ((player.X <= FighterManager.computerPlayers[i].X || player.PROPERTIES.isFacing != Definitions.facing.left) && (player.X >= FighterManager.computerPlayers[i].X || player.PROPERTIES.isFacing != Definitions.facing.right)) || !player.getWhereFistIs().Intersects(FighterManager.computerPlayers[i].getWhereBodyIs()))
				{
					continue;
				}
				if (RandomStaticGlobals.isHardMode)
				{
					int num2 = player.X - FighterManager.computerPlayers[i].X;
					if (num2 < 0)
					{
						num2 *= -1;
					}
					...
					{
						num = num2;
						fighterObject = FighterManager.computerPlayers[i];
					}
				}
				else
				{
					SoundManager.playNextGoreyHitStereo(0f);
					FighterManager.computerPlayers[i].BleedForMe(0, 1f);
					FighterManager.computerPlayers[i].onDeath();
				}
			}
		}
		if (player.PROPERTIES.Attack2After != DateTime.MinValue && player.PROPERTIES.Attack2After < DateTime.Now)
		{
			player.PROPERTIES.Attack2After = DateTime.MinValue;
			for (int j = 0; j < FighterManager.computerPlayers.Count; j++)
			{
				if (!FighterManager.computerPlayers[j].PROPERTIES.isAlive || FighterManager.computerPlayers[j].PROPERTIES.AImode != BunnyOfWar.AI.AI.modes.Y || ((player.X <= FighterManager.computerPlayers[j].X || player.PROPERTIES.isFacing != Definitions.facing.left) && (player.X >= FighterManager.computerPlayers[j].X || player.PROPERTIES.isFacing != Definitions.facing.right)) || !player.getWhereFistIs().Intersects(FighterManager.computerPlayers[j].getWhereBodyIs()))
				{
					continue;
				}
				if (RandomStaticGlobals.isHardMode)
				{
					int num3 = player.X - FighterManager.computerPlayers[j].X;
					if (num3 < 0)
					{
						num3 *= -1;
					}
					...
					{
						num = num3;
						fighterObject = FighterManager.computerPlayers[j];
					}
				}
				else
				{
					SoundManager.playNextGoreyHitStereo(0f);
					FighterManager.computerPlayers[j].BleedForMe(0, 1f);
					FighterManager.computerPlayers[j].onDeath();
				}
			}
		}
		if (RandomStaticGlobals.isHardMode && fighterObject != null)
		{
			SoundManager.playNextGoreyHitStereo(0f);
			fighterObject.BleedForMe(0, 1f);
			fighterObject.onDeath();
		}
	}
	...
	public void processBrawlerInput(FighterObject player, List<FighterObject> enemies)
	{
		...
		if (QuickTimeEventsManager.hasQTE)
		{
			processCutsceneOrQTEInput(player);
			return;
		}
		handleMessagesInput(playerInput, player);
		if (player.PROPERTIES.isDying || !player.PROPERTIES.isAlive)
	...
	public void processTrampolineInput(FighterObject player, List<FighterObject> enemies)
	{
		if (!player.PROPERTIES.areWeHuman)
		{
			BunnyOfWar.AI.AI.doSomething(player, enemies);
		...
		player.PROPERTIES.velocity.Y += 10f;
	}
