...
{
	public static Texture2D imgFailBackground = null;
	...
	private static bool pisShowingCutScene = false;

	private static DateTime pLastShownCutscene = DateTime.MinValue;
	...
	public static bool isShowingCutScene()
	{
		if (pLastShownCutscene.AddMilliseconds(1000.0) > DateTime.Now)
		{
			return true;
		}
		...
		return pisShowingCutScene;
	}

	public static void setIsShowingCutScene(bool show)
	{
		if (!show && pisShowingCutScene)
		{
			pLastShownCutscene = DateTime.Now;
		}
		pisShowingCutScene = show;
	}
	...
	public static void ResetAllGameDefaults()
	{
		...
		setIsShowingCutScene(show: false);
	...
	public static void BuyMe(PlayerIndex pi)
	{
		if (IsTrial())
		{
			...
			{
				GraphicsManager.Message(GraphicsManager.LoadTexture("CutScenes/PurchaseFail", cacheResult: false), 7f);
			}
		}
	}
	...
	public static bool IsThisControllerInputValid(PlayerIndex pi)
	{
		try
		{
			if (Networking.SessionState.HasValue && Networking.SessionState.Value != NetworkSessionState.Playing)
			{
				return true;
			}
	...
	{
		if (GameMode == Definitions.GameMode.cutsceneORqte)
		{
			return;
		}
