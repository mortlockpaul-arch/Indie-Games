...
public static class GraphicsManager
{
	...
	public static Rectangle BoundariesDefault = new Rectangle(0, 0, 1500000, 1500000);
	...
	public static Texture2D imgContinueButton;

	public static Texture2D imgNextButton;
	...
	public static void LoadContent(ContentManager Content)
	{
		...
		imgContinueButton = LoadTexture("buttons/continue.png", cacheResult: false);
		imgNextButton = LoadTexture("buttons/next.png", cacheResult: false);
		...
		string texturePath = "Animations/Blood/bigger.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.2f, isLooping: false, 1f));
		texturePath = "Animations/Blood/bigwedge.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.2f, isLooping: false, 1f));
		texturePath = "Animations/Blood/explosion.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.2f, isLooping: false, 1f));
		texturePath = "Animations/Blood/fatdrops.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.1f, isLooping: false, 1f));
		texturePath = "Animations/Blood/giant splat.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.2f, isLooping: false, 1f));
		texturePath = "Animations/Blood/long.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.2f, isLooping: false, 1f));
		texturePath = "Animations/Blood/longdrops.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.2f, isLooping: false, 1f));
		texturePath = "Animations/Blood/longthin.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.2f, isLooping: false, 1f));
		texturePath = "Animations/Blood/smallerwedge.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.2f, isLooping: false, 1f));
		texturePath = "Animations/Blood/thinbig.png";
		animatedBloodSplatterList.Add(new Animation(texturePath, 0.2f, isLooping: false, 1f));
		bloodStainList.Add(LoadTexture("Animations/BloodStains/2.png", cacheResult: false));
		bloodStainList.Add(LoadTexture("Animations/BloodStains/3.png", cacheResult: false));
		bloodStainList.Add(LoadTexture("Animations/BloodStains/5.png", cacheResult: false));
	}
	...
	public static void DrawLoadingScreen()
	{
		...
		spriteBatch.Draw(LoadTexture("screens/AwesomeEnterprises", cacheResult: true), new Rectangle(0, 0, 1920, 1080), null, Color.White, 0f, Vector2.Zero, SpriteEffects.None, 0f);
		spriteBatch.End();
	}
	...
	public static void Draw(GameTime gameTime)
	{
		...
		if ((!flag && (ScreenManager.CurrentScreen == ScreenManager.screens.Blank || ScreenManager.CurrentScreen == ScreenManager.screens.PauseMenu)) || ScreenManager.isShowingPlayerFailedScreen || ScreenManager.isShowingLevelInBackground)
		{
			...
			{
				FighterObject fighterObject = FighterManager.getHumanPlayerWhoseMostRightForNetworking();
				if (fighterObject == null && FighterManager.humanPlayers.Count > 0)
				{
					fighterObject = FighterManager.humanPlayers[FighterManager.localXboxPlayerID];
				}
			...
			{
				List<FighterObject> computerPlayersAliveOrBleeding = FighterManager.getComputerPlayersAliveOrBleeding();
				foreach (FighterObject item in computerPlayersAliveOrBleeding)
				{
					item.Draw(gameTime, spriteBatch, viewableArea);
				}
				List<FighterObject> computerPlayersWhoDead = FighterManager.getComputerPlayersWhoDead();
				List<string> list = new List<string>(3);
				foreach (FighterObject item2 in computerPlayersWhoDead)
				{
					if (!list.Contains(item2.PROPERTIES.directoryName))
					{
						list.Add(item2.PROPERTIES.directoryName);
					}
				}
				foreach (string item3 in list)
				{
					foreach (FighterObject item4 in computerPlayersWhoDead)
					{
						if (item4.PROPERTIES.directoryName == item3)
						{
							if (item4.PROPERTIES.isFacing == Definitions.facing.right)
							{
								spriteBatch.Draw(item4.animationDying.Texture, item4.getAdjustedXY(viewableArea), null, Color.White, 0f, Vector2.Zero, item4.PROPERTIES.scale, SpriteEffects.None, item4.getLayerDepth());
							}
							else
							{
								spriteBatch.Draw(item4.animationDying.Texture, item4.getAdjustedXY(viewableArea), null, Color.White, 0f, Vector2.Zero, item4.PROPERTIES.scale, SpriteEffects.FlipHorizontally, item4.getLayerDepth());
							}
						}
					}
				}
			...
			{
				foreach (FighterObject item5 in humanPlayers)
				{
					item5.Draw(gameTime, spriteBatch, viewableArea);
				}
			}
			...
			{
				int num = -1;
				foreach (FighterObject item6 in humanPlayers)
				{
					num++;
					...
					Draw(imgSlingshot, new Rectangle(item6.X, item6.Y, 100, 100), null, Color.White, 0f, Vector2.Zero, SpriteEffects.None, 1f);
					if (RandomStaticGlobals.SkullSlingshotCurrentPosition[num] == RandomStaticGlobals.SkullSlingshotOrigin[num])
					{
						Draw(imgSlingshotRopeIdle, new Rectangle(item6.X, item6.Y, 100, 100), null, Color.White, 0f, Vector2.Zero, SpriteEffects.None, 1f);
						continue;
					}
					float rotation = (float)Math.Atan2(RandomStaticGlobals.SkullSlingshotCurrentPosition[num].Y - (float)item6.Y, RandomStaticGlobals.SkullSlingshotCurrentPosition[num].X - (float)item6.X);
					int num2 = (int)RandomStaticGlobals.makePositive(RandomStaticGlobals.SkullSlingshotCurrentPosition[num].X - (float)item6.X);
					int num3 = (int)RandomStaticGlobals.makePositive(RandomStaticGlobals.SkullSlingshotCurrentPosition[num].Y - (float)item6.Y);
					double d = num2 * num2 + num3 * num3;
					d = Math.Sqrt(d);
					Draw(imgSlingshotRopeSlanted, new Rectangle(item6.X + 25, item6.Y + 25, (int)d, 15), null, Color.White, rotation, Vector2.Zero, SpriteEffects.None, Definitions.LayerDepthFifthHighest);
					num2 = (int)RandomStaticGlobals.makePositive(RandomStaticGlobals.SkullSlingshotCurrentPosition[num].X - (float)item6.X - 80f);
					d = num2 * num2 + num3 * num3;
					d = Math.Sqrt(d);
					rotation = (float)Math.Atan2(RandomStaticGlobals.SkullSlingshotCurrentPosition[num].Y - (float)item6.Y, RandomStaticGlobals.SkullSlingshotCurrentPosition[num].X - (float)item6.X - 80f);
					Draw(imgSlingshotRopeSlanted2, new Rectangle(item6.X + 80, item6.Y + 20, (int)d, 15), null, Color.White, rotation, Vector2.Zero, SpriteEffects.None, Definitions.LayerDepthFifthHighest);
				}
			}
			...
			List<FighterObject> computerPlayers = FighterManager.getComputerPlayers(onlyLiving: true, canBeDying: true);
			foreach (FighterObject item7 in computerPlayers)
			{
				item7.Draw(gameTime, spriteBatch, viewableArea);
			}
			spriteBatch.End();
		}
		else
	...
	public static void ShowCutSceneBtoContinue(string sceneName, string uniqueName, int durationMS, bool isSkippable)
	{
		RandomStaticGlobals.setIsShowingCutScene(show: true);
		MessageBoxScreen messageBoxScreen = new MessageBoxScreen(LoadTexture("CutScenes/" + sceneName), (float)durationMS / 1000f, isQTEa: false, isCutScenea: true, sceneName, uniqueName);
	...
	public static void ShowCutScene(string sceneName, string uniqueName, int durationMS, bool isSkippable)
	{
		RandomStaticGlobals.setIsShowingCutScene(show: true);
		MessageBoxScreen messageBoxScreen = new MessageBoxScreen(LoadTexture("CutScenes/" + sceneName), (float)durationMS / 1000f, isQTEa: false, isCutScenea: true, sceneName, uniqueName);
		messageBoxScreen.isSkippable = isSkippable;
	...
	public static void ShowQuickTimeEvent(string sceneName, string uniqueName, int durationMS)
	{
		RandomStaticGlobals.setIsShowingCutScene(show: true);
		messages.Add(new MessageBoxScreen(LoadTexture("CutScenes/" + sceneName), (float)durationMS / 1000f, isQTEa: true, isCutScenea: true, sceneName, uniqueName));
		messages.RemoveRange(0, messages.Count - 1);
	}
	...
	public static bool DrawMessages()
	{
		...
		if (!flag && messages.Count > 0)
		{
			...
			{
				if (!messages[j].isActive)
				{
					messages[j].isActive = true;
					messages[j].isQueuedUp = false;
					messages[j].startTimer();
					messages[j].Draw();
					return flag;
				}
			}
	...
	public static void DrawHealth()
	{
		...
		if (FighterManager.humanPlayers.Count > 0 && FighterManager.humanPlayers[0] != null)
		{
			...
			DrawRectangle(new Rectangle((int)titleSafeTopLeft.X + 5, (int)titleSafeTopLeft.Y + 5, (int)FighterManager.humanPlayers[0].PROPERTIES.health * 5, 30), Color.DarkRed, Definitions.LayerDepthFourthHighest);
			DrawRectangle(new Rectangle((int)titleSafeTopLeft.X, (int)titleSafeTopLeft.Y, (int)FighterManager.humanPlayers[0].PROPERTIES.healthMax * 5 + 10, 40), TheColorTransparentGray, Definitions.LayerDepthFifthHighest);
	...
	public static void ClearTextureCache()
	{
		...
		{
			RandomStaticGlobals.ContentTemporary.Unload();
		}
	}
