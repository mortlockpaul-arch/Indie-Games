...
public class FighterObjectRangedCode
{
	...
	public void rangedTongueAttack(FighterObject enemy)
	{
		...
		Vector2 vector2 = new Vector2(RandomStaticGlobals.makePositive(direction.X), RandomStaticGlobals.makePositive(direction.Y));
		if (vector2.X > vector2.Y)
		{
			direction.X /= vector2.X;
			if (direction.Y != 0f)
			{
				direction.Y /= vector2.X;
			}
		}
		else
		{
			direction.Y /= vector2.Y;
			if (direction.X != 0f)
			{
				direction.X /= vector2.Y;
			}
		}
