...
public class PauseMenu
{
	...
	private void FigureOutInput(InputFromAnywhere anywhereInput, PlayerIndex pi)
	{
		if (anywhereInput != null)
		{
			...
			if (anywhereInput.START_pressed)
			{
				RandomStaticGlobals.pauseButtonPressed(broadcastThis: true);
			}
		}
	...
	public void Draw()
	{
		TriggerManager.AddTimeFromPause();
		try
		{
			GraphicsManager.Draw(GraphicsManager.LoadTexture("screens/Pause.png", cacheResult: true), new Rectangle(0, 0, 1920, 1080), null, Color.White, 0f, Vector2.Zero, SpriteEffects.None, Definitions.LayerDepthSecondHighest);
		}
		catch (Exception)
		{
		}
