...
{
	private bool controllerOneReady;

	private bool controllerTwoReady;

	private bool controllerThreeReady;

	private bool controllerFourReady;

	private bool remotePlayerOneReady;

	private bool remotePlayerTwoReady;

	private bool remotePlayerThreeReady;

	private bool remotePlayerFourReady;

	private bool remotePlayerFiveReady;

	private bool remotePlayerSixReady;

	private bool remotePlayerSevenReady;

	private bool remotePlayerEightReady;
	...
	public void Draw()
	{
		...
		foreach (NetworkGamer allGamer in Networking.AllGamers)
		{
			GraphicsManager.DrawString(num3, num + num4, allGamer.Gamertag, Color.DarkRed, GraphicsManager.fontSmall);
			num4 = ((num4 != 350) ? 350 : 0);
			num3 += 200;
		}
	}
	...
	public void ProcessInput()
	{
		...
		if (GamePad.GetState(PlayerIndex.One).IsConnected)
		{
			...
			{
				text = Gamer.SignedInGamers[PlayerIndex.One].Gamertag;
			}
		...
		if (GamePad.GetState(PlayerIndex.Two).IsConnected)
		{
			...
			{
				text = Gamer.SignedInGamers[PlayerIndex.Two].Gamertag;
			}
		...
		if (GamePad.GetState(PlayerIndex.Three).IsConnected)
		{
			...
			{
				text = Gamer.SignedInGamers[PlayerIndex.Three].Gamertag;
			}
		...
		if (GamePad.GetState(PlayerIndex.Four).IsConnected)
		{
			...
			{
				text = Gamer.SignedInGamers[PlayerIndex.Four].Gamertag;
			}
		...
		if (Networking.SessionState == NetworkSessionState.Playing)
		{
			startGameClient();
		}
	}
	...
	public void startGame()
	{
		...
		if (Networking.isHost || Networking.RemoteGamersCount == 0)
		{
			...
			if (controllerOneReady && GamePad.GetState(PlayerIndex.One).IsConnected)
			{
				...
				if (Gamer.SignedInGamers[PlayerIndex.One] != null)
				{
					fighterObject = FighterManager.addNewHumanPlayer(PlayerIndex.One, isNetworkPlayer: false, Gamer.SignedInGamers[PlayerIndex.One].Gamertag, scale);
					fighterObject.PROPERTIES.GamerTag = Gamer.SignedInGamers[PlayerIndex.One].Gamertag;
				}
				else
				{
			...
			if (controllerTwoReady && GamePad.GetState(PlayerIndex.Two).IsConnected)
			{
				...
				if (Gamer.SignedInGamers[PlayerIndex.Two] != null)
				{
					fighterObject = FighterManager.addNewHumanPlayer(PlayerIndex.Two, isNetworkPlayer: false, Gamer.SignedInGamers[PlayerIndex.Two].Gamertag, scale);
					fighterObject.PROPERTIES.GamerTag = Gamer.SignedInGamers[PlayerIndex.Two].Gamertag;
				}
				else
				{
			...
			if (controllerThreeReady && GamePad.GetState(PlayerIndex.Three).IsConnected)
			{
				...
				if (Gamer.SignedInGamers[PlayerIndex.Three] != null)
				{
					fighterObject = FighterManager.addNewHumanPlayer(PlayerIndex.Three, isNetworkPlayer: false, Gamer.SignedInGamers[PlayerIndex.Three].Gamertag, scale);
					fighterObject.PROPERTIES.GamerTag = Gamer.SignedInGamers[PlayerIndex.Three].Gamertag;
				}
				else
				{
			...
			if (controllerFourReady && GamePad.GetState(PlayerIndex.Four).IsConnected)
			{
				...
				if (Gamer.SignedInGamers[PlayerIndex.Four] != null)
				{
					fighterObject = FighterManager.addNewHumanPlayer(PlayerIndex.Four, isNetworkPlayer: false, Gamer.SignedInGamers[PlayerIndex.Four].Gamertag, scale);
					fighterObject.PROPERTIES.GamerTag = Gamer.SignedInGamers[PlayerIndex.Four].Gamertag;
				}
				else
				{
			...
			if (Networking.RemoteGamersCount > 0)
			{
				foreach (NetworkGamer remoteGamer in Networking.RemoteGamers)
				{
					FighterManager.addNewHumanPlayer(null, isNetworkPlayer: true, remoteGamer.Gamertag, scale);
				}
			}
	...
	private void FigureOutInput(GamePadState? gamePadState, GamePadState? previousGamePadState, ref bool readyVar, PlayerIndex ndx, string gamerTag)
	{
		...
		if (gamePadState.Value.Buttons.Y == ButtonState.Released && previousGamePadState.Value.Buttons.Y == ButtonState.Pressed)
		{
			...
			{
				Networking.ListPublicGames(NetworkSessionType.PlayerMatch, ndx, gamerTag);
			}
			else
			{
	...
	private static void SendNetworkPlayerReady(Gamer gamer, bool isReady)
	{
		...
		{
			Networking.packetWriter.Write((byte)12);
			Networking.packetWriter.Write(gamer.Gamertag.Length);
			Networking.packetWriter.Write(gamer.Gamertag.ToCharArray());
			Networking.packetWriter.Write(isReady);
