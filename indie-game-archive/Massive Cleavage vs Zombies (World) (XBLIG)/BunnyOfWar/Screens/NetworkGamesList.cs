...
public class NetworkGamesList
{
	...
	public void Draw()
	{
		...
		new Rectangle((int)Math.Min(vector3.X, xBOX_TEXT_POSITION.X), (int)xBOX_TEXT_POSITION.Y, (int)Math.Max(vector2.X, vector.X), (int)((float)GraphicsManager.font.LineSpacing * 1.1f + vector2.Y));
		int num = 0;
		if (currentPage > 0)
		{
		...
		for (int i = 0; i < showXGamesPerPage && i + num < Networking.multiplayerAvailableGamesList.Count; i++)
		{
			...
			Rectangle rectangle = new Rectangle((int)xBOX_TEXT_POSITION.X, (int)xBOX_TEXT_POSITION.Y + i * 60, (int)vector4.X, (int)vector4.Y);
			rectangle.Inflate(50, 10);
			GraphicsManager.DrawRectangle(rectangle, GraphicsManager.TheColorTransparentGray);
		...
		Rectangle rectangle2 = new Rectangle(backButtonRect.X, backButtonRect.Y, (int)vector5.X, (int)vector5.Y);
		rectangle2.Inflate(50, 10);
		GraphicsManager.DrawRectangle(rectangle2, GraphicsManager.TheColorTransparentGray, Definitions.LayerDepthSecondHighest);
		GraphicsManager.DrawRectangle(rectangle2, GraphicsManager.TheColorTransparentGray, Definitions.LayerDepthSecondHighest);
		GraphicsManager.DrawString(backButtonRect.X, backButtonRect.Y, text, Color.White, GraphicsManager.font);
		GraphicsManager.Draw(GraphicsManager.imgButtonB, new Rectangle(rectangle2.X - 70, rectangle2.Y - 30, 100, 100), null, Color.White, 0f, Vector2.Zero, SpriteEffects.None, Definitions.LayerDepthTop);
	}
