...
public static class SceneryManager
{
	...
	public static void DrawScenerySkyAndGround()
	{
		...
		for (int j = 0; j < BackGroundSceneryObjects.Count; j++)
		{
			if (BackGroundSceneryObjects[j].isVisible)
			{
				SpriteEffects effects2 = SpriteEffects.None;
				if (BackGroundSceneryObjects[j].isFlippedHorizontally)
				{
					effects2 = SpriteEffects.FlipHorizontally;
				}
				if (BackGroundSceneryObjects[j].isFlippedVertically)
				{
					effects2 = SpriteEffects.FlipVertically;
				}
				if (BackGroundSceneryObjects[j].AImode == NonFighterAI.modes.ScrollRatioToPlayer)
				{
					GraphicsManager.Draw(GraphicsManager.LoadTexture("scenery/ground/" + BackGroundSceneryObjects[j].name), GraphicsManager.getAdjustedRectangleForSlowScroller(BackGroundSceneryObjects[j].rect, BackGroundSceneryObjects[j].AIAmountSpeed), null, Color.White, 0f, Vector2.Zero, effects2, Definitions.LayerDepthForGround);
				}
				else if (BackGroundSceneryObjects[j].rect.Intersects(GraphicsManager.viewableArea))
				{
					GraphicsManager.Draw(GraphicsManager.LoadTexture("scenery/ground/" + BackGroundSceneryObjects[j].name), GraphicsManager.getAdjustedRectangle(BackGroundSceneryObjects[j].rect), null, Color.White, 0f, Vector2.Zero, effects2, Definitions.LayerDepthForGround);
				}
			}
		}
	}
	...
	public static void DrawSceneryFloaters()
	{
		...
		{
			if (!FloatingSceneryObjects[i].isVisible)
			{
				continue;
			}
			flag = true;
		...
		white.A = 200;
		for (int j = 0; j < BloodStainSceneryObjects.Count; j++)
		{
			if (!BloodStainSceneryObjects[j].rect.Intersects(GraphicsManager.viewableArea))
			{
				continue;
			}
			if (Definitions.Options.BloodOnOff)
			{
				...
				{
					GraphicsManager.Draw(GraphicsManager.bloodStainList[int.Parse(BloodStainSceneryObjects[j].name)], GraphicsManager.getAdjustedRectangle(BloodStainSceneryObjects[j].rect), null, white, 0f, Vector2.Zero, SpriteEffects.None, Definitions.LayerDepthForBlood);
				}
			}
			else
			{
				GraphicsManager.Draw(GraphicsManager.bloodStainListGREEN[int.Parse(BloodStainSceneryObjects[j].name)], GraphicsManager.getAdjustedRectangle(BloodStainSceneryObjects[j].rect), null, white, 0f, Vector2.Zero, SpriteEffects.None, Definitions.LayerDepthForBlood);
			}
		}
	}
	...
	public static void ActivateNamedObject(string name)
	{
		...
		for (int j = 0; j < BackGroundSceneryObjects.Count; j++)
		{
			if (BackGroundSceneryObjects[j].uniqueName == name)
			{
				BackGroundSceneryObjects[j].isVisible = !BackGroundSceneryObjects[j].isVisible;
			}
		}
		for (int k = 0; k < FloatingSceneryObjects.Count; k++)
		{
			if (FloatingSceneryObjects[k].uniqueName == name)
			{
				FloatingSceneryObjects[k].isVisible = !FloatingSceneryObjects[k].isVisible;
			}
		}
	}
