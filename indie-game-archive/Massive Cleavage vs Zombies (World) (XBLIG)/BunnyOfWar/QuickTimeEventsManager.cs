...
public static class QuickTimeEventsManager
{
	...
	public static void AddQTE(string QTEName, string sceneName, int durationInMS, QTEButtons buttons)
	{
		...
		GraphicsManager.ShowQuickTimeEvent(sceneName, QTEName, durationInMS);
	}
	...
	public static void Draw()
	{
		...
		float num3 = (float)num2 / (float)QTE.durationInMS;
		int num4 = (int)(num3 * GraphicsManager.ScreenFullSize.X);
		int x = ((int)GraphicsManager.ScreenFullSize.X - num4) / 2;
		GraphicsManager.DrawRectangle(new Rectangle(x, 780, num4, 100), Color.Red, Definitions.LayerDepthTop);
	}
}
