...
{
	public FighterObjectProperties PROPERTIES;

	public FighterObjectRangedCode RANGED;

	public FighterObjectJumpCode JUMP;
	...
	private static int staticY = -1;

	private static int[] staticYRandoms = new int[300];
	...
	public void explodeOnMe(FighterObject enemy, int damage)
	{
		...
		_ = num / (float)GraphicsManager.viewportRect.Width;
	...
	public void onDeath(bool broadcast)
	{
		...
		{
			X += random.Next(-300, -50);
		}
		if (staticY == -1)
		{
			for (int i = 0; i < staticYRandoms.Length; i++)
			{
				staticYRandoms[i] = i;
			}
			for (int j = 0; j < staticYRandoms.Length; j++)
			{
				int num = (staticYRandoms[j] = random.Next(0, staticYRandoms.Length));
				int num2 = staticYRandoms[num];
				staticYRandoms[num] = staticYRandoms[j];
				staticYRandoms[j] = num2;
			}
		}
		staticY++;
		if (staticY >= staticYRandoms.Length)
		{
			staticY = 0;
		}
		Y += staticYRandoms[staticY];
		if (broadcast)
		{
	...
	public void onIdle(bool force)
	{
		...
		if (!PROPERTIES.isWalking && !(PROPERTIES.Attack1After > DateTime.Now) && !(PROPERTIES.Attack2After > DateTime.Now) && !(PROPERTIES.ChangeAnimationAfter > DateTime.Now))
		{
	...
	public void BleedForMe(int bloodStains, float bloodSizePercentage)
	{
		...
		int num = 0;
		Random random = new Random(DateTime.Now.Millisecond);
		num = random.Next(GraphicsManager.animatedBloodSplatterList.Count - 1);
		...
		if (bloodSizePercentage == 1f)
		{
			...
			if (PROPERTIES.isFacing == Definitions.facing.left)
			{
				PROPERTIES.bloodX = X + width / 2 + GraphicsManager.animatedBloodSplatterList[num].FrameHeight / 2;
			}
			else
			{
				PROPERTIES.bloodX = X - width / 2 - GraphicsManager.animatedBloodSplatterList[num].FrameHeight / 2;
			...
			if (PROPERTIES.isFacing == Definitions.facing.left)
			{
				PROPERTIES.bloodX = whereBodyIs.X + GraphicsManager.animatedBloodSplatterList[num].FrameHeight / 2;
			}
			else
			{
				PROPERTIES.bloodX = whereBodyIs.X - whereBodyIs.Width - GraphicsManager.animatedBloodSplatterList[num].FrameHeight / 2;
			}
		...
		{
			int num2 = (int)((float)rectSpriteDisplay.Height * bloodSizePercentage);
			int num3 = (int)((float)rectSpriteDisplay.Width * bloodSizePercentage);
			PROPERTIES.bloodX = rectSpriteDisplay.X;
			PROPERTIES.bloodY = rectSpriteDisplay.Y;
			PROPERTIES.spriteBlood.rectangleResizeToThis = new Rectangle(rectSpriteDisplay.X, rectSpriteDisplay.Y, num3, num2);
		}
		PROPERTIES.spriteBlood.PlayAnimation(GraphicsManager.animatedBloodSplatterList[num], forceRestart: true);
