...
public class Bird : IGameObject, IAnimation, IExplosion
{
	...
	public static void ResetAllBirdPositions(List<Bird> birds, GraphicsDevice graphicsDevice)
	{
		for (int i = 0; i < birds.Count; i++)
		{
			birds[i].Position = new Vector2((float)graphicsDevice.Viewport.Width - (float)graphicsDevice.Viewport.Width * 0.03f + 250f, 330f - (float)(i * 30));
			birds[i].Dead = false;
		}
	}
}
