...
public class DialogMenuBuild : DialogMenu
{
	private static Color COLOR_TITLE = new Color(147, 143, 139);

	private static Color COLOR_TITLE_SHADOW = new Color(25, 24, 23);

	private static Color COLOR_DESC_SHADOW = new Color(50, 48, 46);
	...
	public DialogMenuBuild(DialogManager oManager, string xTitle, string xDesc, List<DialogMenuOption> aOptions, List<DialogMenuButtonLable> aLables)
	...
	public override void Load()
	{
		...
		background.texture = GameEngine.Content.Load<Texture2D>("Content/UI/Dialogs/Menu Dialogs/Build/Background");
		logo = new Sprite(manager.spriteManager);
		logo.texture = GameEngine.Content.Load<Texture2D>("Content/UI/Dialogs/Menu Dialogs/Build/Logo");
		base.Load();
		Lables_SetColors(COLOR_TITLE, COLOR_TITLE_SHADOW);
	...
	public override void Show()
	{
		...
		logo.position.X = (float)x + ((float)width2 - logo.size.X) * 0.5f;
