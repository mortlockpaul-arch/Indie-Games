...
public class PostProcess_BloomSimple : PostProcess, IEditable
{
	...
	private const string PATH = "Content/Effects/Post/Post_BloomSimple";
	...
	public PostProcess_BloomSimple(EntityStack oEntityStack)
		: base(oEntityStack)
	{
	}
	...
	{
		base.Load();
		effect = GameEngine.SceneContent.Load<Effect>("Content/Effects/Post/Post_BloomSimple");
		...
		SetBlurEffectParameters(5f / (float)GameEngine.Graphics.GraphicsDevice.Viewport.Width, 0f);
	}
	...
	public override void Execute(GraphicsDevice oDevice, GameTime oGameTime)
	{
		...
		targetIndex = GameEngine.instance.renderer.targetIndex;
		GameEngine.instance.renderer.ToggleTarget();
		SetBlurEffectParameters(0f, 5f / (float)GameEngine.Graphics.GraphicsDevice.Viewport.Height);
		_pass = effect.CurrentTechnique.Passes[0];
		GameEngine.instance.renderer.spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.Opaque);
		_pass.Apply();
		GameEngine.instance.renderer.spriteBatch.Draw(GameEngine.instance.renderer.target[targetIndex], position, Color.White);
