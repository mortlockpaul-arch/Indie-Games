...
public class BroadPhase
{
	...
	{
		int num = _tree.CreateProxy(ref aabb, userData);
		_proxyCount++;
		BufferMove(num);
		return num;
	}
	...
	public void UpdatePairs<T>(BroadphaseDelegate callback)
	{
		...
		while (j < _pairCount)
		{
			...
			FixtureProxy proxyA = _tree.GetUserData<FixtureProxy>(pair.ProxyIdA);
			FixtureProxy proxyB = _tree.GetUserData<FixtureProxy>(pair.ProxyIdB);
