...
public static class Distance
{
	...
	public static void ComputeDistance(out DistanceOutput output, out SimplexCache cache, ref DistanceInput input)
	{
		...
		while (num3 < 20)
		{
			...
			value.WA = MathUtils.Multiply(ref input.TransformA, input.ProxyA.GetVertex(value.IndexA));
			value.IndexB = input.ProxyB.GetSupport(MathUtils.MultiplyT(ref input.TransformB.R, searchDirection));
			value.WB = MathUtils.Multiply(ref input.TransformB, input.ProxyB.GetVertex(value.IndexB));
