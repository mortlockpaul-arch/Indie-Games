...
public struct SeparationFunction
{
	...
	public SeparationFunction(ref SimplexCache cache, ref DistanceProxy proxyA, ref Sweep sweepA, ref DistanceProxy proxyB, ref Sweep sweepB, float t1)
	{
		...
		else if (cache.IndexA[0] == cache.IndexA[1])
		{
			...
			_axis = MathUtils.Cross(vertex4 - vertex3, 1f);
			_axis.Normalize();
			Vector2 value = MathUtils.Multiply(ref xf2.R, _axis);
			_localPoint = 0.5f * (vertex3 + vertex4);
			Vector2 vector3 = MathUtils.Multiply(ref xf2, _localPoint);
			Vector2 vertex5 = proxyA.GetVertex(cache.IndexA[0]);
			Vector2 vector4 = MathUtils.Multiply(ref xf, vertex5);
			float num = Vector2.Dot(vector4 - vector3, value);
			if (num < 0f)
			{
				_axis = -_axis;
				num = 0f - num;
		...
		else
		{
			...
			_axis = MathUtils.Cross(vertex7 - vertex6, 1f);
			_axis.Normalize();
			...
			Vector2 vector6 = MathUtils.Multiply(ref xf2, vertex8);
			float num2 = Vector2.Dot(vector6 - vector5, value2);
			if (num2 < 0f)
			{
				_axis = -_axis;
				num2 = 0f - num2;
