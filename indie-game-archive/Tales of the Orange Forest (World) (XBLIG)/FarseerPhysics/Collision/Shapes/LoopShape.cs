...
public class LoopShape : Shape
{
	...
	public override Shape Clone()
	{
		...
		loopShape.Count = Count;
		loopShape.Radius = Radius;
		loopShape.Vertices = (Vector2[])Vertices.Clone();
		return loopShape;
	}
	...
	public void GetChildEdge(ref EdgeShape edge, int index)
	{
		...
		int num = ((index - 1 >= 0) ? (index - 1) : (Count - 1));
		int num2 = ((index + 1 < Count) ? (index + 1) : 0);
		int num3;
		for (num3 = index + 2; num3 >= Count; num3 -= Count)
		{
		}
		edge.Vertex0 = Vertices[num];
		edge.Vertex1 = Vertices[index];
		edge.Vertex2 = Vertices[num2];
		edge.Vertex3 = Vertices[num3];
	}
