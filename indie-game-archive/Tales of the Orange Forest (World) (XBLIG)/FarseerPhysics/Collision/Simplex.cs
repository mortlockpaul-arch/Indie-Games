...
internal struct Simplex
{
	...
	internal void ReadCache(ref SimplexCache cache, ref DistanceProxy proxyA, ref Transform transformA, ref DistanceProxy proxyB, ref Transform transformB)
	{
		...
		for (int i = 0; i < Count; i++)
		{
			...
			Vector2 vertex = proxyA.GetVertex(value.IndexA);
			Vector2 vertex2 = proxyB.GetVertex(value.IndexB);
			value.WA = MathUtils.Multiply(ref transformA, vertex);
			value.WB = MathUtils.Multiply(ref transformB, vertex2);
		...
		if (Count == 0)
		{
			...
			value2.IndexA = 0;
			value2.IndexB = 0;
			Vector2 vertex3 = proxyA.GetVertex(0);
			Vector2 vertex4 = proxyB.GetVertex(0);
			value2.WA = MathUtils.Multiply(ref transformA, vertex3);
			value2.WB = MathUtils.Multiply(ref transformB, vertex4);
	...
	internal Vector2 GetSearchDirection()
	{
		...
		case 2:
		{
			...
			if (num > 0f)
			{
				return MathUtils.Cross(1f, a);
			}
			return MathUtils.Cross(a, 1f);
		}
		default:
			return Vector2.Zero;
		}
