...
public static class YuPengClipper
{
	private sealed class Edge
	{
		...
		public override bool Equals(object obj)
		{
			...
			if (EdgeStart == edge.EdgeStart)
			{
				return EdgeEnd == edge.EdgeEnd;
			}
			return false;
		}
		...
		public bool Equals(Edge e)
		{
			...
			if (EdgeStart == e.EdgeStart)
			{
				return EdgeEnd == e.EdgeEnd;
			}
			return false;
		}
	...
	private static List<Vertices> Execute(Vertices subject, Vertices clip, PolyClipType clipType, out PolyClipError error)
	{
		...
		CalculateEdgeCharacter(coeff, simplicies, coeff2, simplicies2, out var poly1Char, out var poly2Char);
		CalculateResultChain(simplicies, poly1Char, simplicies2, poly2Char, clipType, out var resultSimplices);
	...
	private static void CalculateEdgeCharacter(List<float> poly1Coeff, List<Edge> poly1Simplicies, List<float> poly2Coeff, List<Edge> poly2Simplicies, out List<float> poly1Char, out List<float> poly2Char)
	{
		...
		for (int i = 0; i < poly1Simplicies.Count; i++)
		{
			...
			for (int j = 0; j < poly2Simplicies.Count; j++)
			{
				if (!poly2Simplicies.Contains(-poly1Simplicies[i]))
				{
					poly1Char[i] += CalculateBeta(poly1Simplicies[i].GetCenter(), poly2Simplicies[j], poly2Coeff[j]);
				}
			}
		}
		for (int k = 0; k < poly2Simplicies.Count; k++)
		{
			...
			for (int l = 0; l < poly1Simplicies.Count; l++)
			{
				...
				{
					poly2Char[k] += CalculateBeta(poly2Simplicies[k].GetCenter(), poly1Simplicies[l], poly1Coeff[l]);
				}
			}
		}
	}

	private static void CalculateResultChain(List<Edge> poly1Simplicies, List<float> poly1Char, List<Edge> poly2Simplicies, List<float> poly2Char, PolyClipType clipType, out List<Edge> resultSimplices)
	{
	...
	private static float CalculateBeta(Vector2 point, Edge e, float coefficient)
	{
		...
		if (PoinOnLineSegment(Vector2.Zero, e.EdgeStart, point) || PoinOnLineSegment(Vector2.Zero, e.EdgeEnd, point))
		{
	...
	private static bool PointInSimplex(Vector2 point, Edge edge)
	{
		...
		return vertices.PointInPolygon(ref point);
	}
