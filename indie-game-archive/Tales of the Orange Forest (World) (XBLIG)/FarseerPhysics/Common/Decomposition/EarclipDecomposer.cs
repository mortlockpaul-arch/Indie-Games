...
public static class EarclipDecomposer
{
		...
		while (flag)
		{
			...
			while (num4 < 2 * triangulated.Count)
			{
				...
				if (!array[num3])
				{
					...
					if (vertices2 != null && vertices2.Count <= 8 && vertices2.IsConvex())
					{
			...
			if (num < maxPolys)
			{
				SimplifyTools.MergeParallelEdges(vertices, tolerance);
				if (vertices.Count >= 3)
				{
	...
	public static List<Triangle> TriangulatePolygon(Vertices vertices)
	{
		...
		while (num2 > 3)
		{
			...
			Triangle t = new Triangle(array2[num3], array3[num3], array2[num10], array3[num10], array2[num9], array3[num9]);
			array[num] = new Triangle(t);
			num++;
			array2 = array4;
			array3 = array5;
		}
		Triangle t2 = new Triangle(array2[1], array3[1], array2[2], array3[2], array2[0], array3[0]);
		array[num] = new Triangle(t2);
		num++;
