...
public class PulleyJoint : Joint
{
	...
	public PulleyJoint(Body bodyA, Body bodyB, Vector2 groundAnchorA, Vector2 groundAnchorB, Vector2 anchorA, Vector2 anchorB, float ratio)
		: base(bodyA, bodyB)
	{
		...
		_lengthA = (base.BodyA.GetWorldPoint(anchorA) - groundAnchorA).Length();
		_lengthB = (base.BodyB.GetWorldPoint(anchorB) - groundAnchorB).Length();
		Ratio = ratio;
		float num = _lengthA + Ratio * _lengthB;
		_maxLengthA = num - Ratio * MinPulleyLength;
		_maxLengthB = (num - MinPulleyLength) / Ratio;
