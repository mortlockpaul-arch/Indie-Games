...
public class FixedDistanceJoint : Joint
{
	...
	public sealed override Vector2 WorldAnchorB => LocalAnchorB;
	...
	internal override void InitVelocityConstraints(ref TimeStep step)
	{
		...
		_u = localAnchorB - bodyA.Sweep.c - vector;
		float num = _u.Length();
		if (num > 0.005f)
		{
			_u *= 1f / num;
		...
		float num3 = MathUtils.Cross(localAnchorB, _u);
	...
	internal override void SolveVelocityConstraints(ref TimeStep step)
	{
		...
		float num = Vector2.Dot(value2: new Vector2(0f, 0f) - vector, value1: _u);
	...
	internal override bool SolvePositionConstraints()
	{
		...
		Vector2 vector = MathUtils.Multiply(ref transform.R, LocalAnchorA - bodyA.LocalCenter);
		Vector2 localAnchorB = LocalAnchorB;
		Vector2 u = localAnchorB - bodyA.Sweep.c - vector;
		float num = u.Length();
		if (num == 0f)
		{
			return true;
