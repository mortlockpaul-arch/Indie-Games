...
public class FixedRevoluteJoint : Joint
{
	...
	public override Vector2 WorldAnchorB => LocalAnchorB;
	...
	internal override void InitVelocityConstraints(ref TimeStep step)
	{
		...
		Vector2 localAnchorB = LocalAnchorB;
		float invMass = bodyA.InvMass;
		float invI = bodyA.InvI;
		_mass.col1.X = invMass + 0f + a.Y * a.Y * invI + localAnchorB.Y * localAnchorB.Y * 0f;
		_mass.col2.X = (0f - a.Y) * a.X * invI - localAnchorB.Y * localAnchorB.X * 0f;
		_mass.col3.X = (0f - a.Y) * invI - localAnchorB.Y * 0f;
		_mass.col1.Y = _mass.col2.X;
		_mass.col2.Y = invMass + 0f + a.X * a.X * invI + localAnchorB.X * localAnchorB.X * 0f;
		_mass.col3.Y = a.X * invI + localAnchorB.X * 0f;
	...
	internal override void SolveVelocityConstraints(ref TimeStep step)
	{
		...
		if (_enableLimit && _limitState != LimitState.Inactive)
		{
			...
			Vector2 a = MathUtils.Multiply(ref transform.R, LocalAnchorA - bodyA.LocalCenter);
			Vector2 localAnchorB = LocalAnchorB;
			Vector2 vector = zero + MathUtils.Cross(0f, localAnchorB) - linearVelocityInternal - MathUtils.Cross(num, a);
			float z = 0f - num;
		...
		else
		{
			...
			Vector2 localAnchorB2 = LocalAnchorB;
			Vector2 vector7 = zero + MathUtils.Cross(0f, localAnchorB2) - linearVelocityInternal - MathUtils.Cross(num, a2);
	...
	internal override bool SolvePositionConstraints()
	{
		...
		Vector2 localAnchorB = LocalAnchorB;
		Vector2 vector2 = Vector2.Zero + localAnchorB - bodyA.Sweep.c - vector;
		...
		if (vector2.LengthSquared() > 0.0025f)
		{
			...
			vector2 = Vector2.Zero + localAnchorB - bodyA.Sweep.c - vector;
		}
		...
		Mat22 B2 = new Mat22(new Vector2(0f * localAnchorB.Y * localAnchorB.Y, -0f * localAnchorB.X * localAnchorB.Y), new Vector2(-0f * localAnchorB.X * localAnchorB.Y, 0f * localAnchorB.X * localAnchorB.X));
