...
public class CompteurSpad
{
	...
	public CompteurSpad(CustomPhysicsGame game)
	{
		timecounterSA = 0;
		timecounterMA = 0;
		timecounterHA = 0;
	...
	public void UpdateTime(CustomPhysicsGame game, GameTime gameTime)
	{
		...
		checked
		{
			if (game.avion1.altitude >= 2f && !game.avion1.Avioncasse && game.gamemode == CustomPhysicsGame.GameMode.M0)
			{
			...
			if (totalCHi <= totalCH)
			{
				...
				totalCrash = game.avion1.compteCrash;
				if (game.avion1.compteCrash <= totalCrash)
				{
					totalCrash = game.avion1.compteCrash;
				}
			}
