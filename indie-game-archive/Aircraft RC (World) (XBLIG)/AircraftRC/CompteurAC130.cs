...
public class CompteurAC130
{
	...
	public CompteurAC130(CustomPhysicsGame game)
	{
		timecounterSA = 0;
		timecounterMA = 0;
		timecounterHA = 0;
	...
	public void UpdateTime(CustomPhysicsGame game, GameTime gameTime)
	{
		...
		checked
		{
			if (game.avion5.altitude >= 2f && !game.avion5.Avioncasse && game.gamemode == CustomPhysicsGame.GameMode.M0)
			{
			...
			if (totalCHi <= totalCH)
			{
				...
				totalCrash = game.avion5.compteCrash;
				if (game.avion5.compteCrash <= totalCrash)
				{
					totalCrash = game.avion5.compteCrash;
				}
			}
