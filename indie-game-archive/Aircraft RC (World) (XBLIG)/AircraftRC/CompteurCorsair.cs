...
public class CompteurCorsair
{
	...
	public CompteurCorsair(CustomPhysicsGame game)
	{
		timecounterSA = 0;
		timecounterMA = 0;
		timecounterHA = 0;
	...
	public void UpdateTime(CustomPhysicsGame game, GameTime gameTime)
	{
		...
		checked
		{
			if (game.avion2.altitude >= 2f && !game.avion2.Avioncasse && game.gamemode == CustomPhysicsGame.GameMode.M0)
			{
			...
			if (totalCHi <= totalCH)
			{
				...
				totalCrash = game.avion2.compteCrash;
				if (game.avion2.compteCrash <= totalCrash)
				{
					totalCrash = game.avion2.compteCrash;
				}
			}
