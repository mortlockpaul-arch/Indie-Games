...
public class CompteurDH112
{
	...
	public CompteurDH112(CustomPhysicsGame game)
	{
		timecounterSA = 0;
		timecounterMA = 0;
		timecounterHA = 0;
	...
	public void UpdateTime(CustomPhysicsGame game, GameTime gameTime)
	{
		...
		checked
		{
			if (game.avion3.altitude >= 2f && !game.avion3.Avioncasse && game.gamemode == CustomPhysicsGame.GameMode.M0)
			{
			...
			if (totalCHi <= totalCH)
			{
				...
				totalCrash = game.avion3.compteCrash;
				if (game.avion3.compteCrash <= totalCrash)
				{
					totalCrash = game.avion3.compteCrash;
				}
			}
