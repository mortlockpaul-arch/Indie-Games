...
public class CompteurCanadair
{
	...
	public CompteurCanadair(CustomPhysicsGame game)
	{
		timecounterSA = 0;
		timecounterMA = 0;
		timecounterHA = 0;
	...
	public void UpdateTime(CustomPhysicsGame game, GameTime gameTime)
	{
		...
		checked
		{
			if (game.avion4.altitude >= 2f && !game.avion4.Avioncasse && game.gamemode == CustomPhysicsGame.GameMode.M0)
			{
			...
			if (totalCHi <= totalCH)
			{
				...
				totalCrash = game.avion4.compteCrash;
				if (game.avion4.compteCrash <= totalCrash)
				{
					totalCrash = game.avion4.compteCrash;
				}
			}
