...
/// </summary>
public class ContactCollection : IList<ContactInformation>, ICollection<ContactInformation>, IEnumerable<ContactInformation>, IEnumerable
{
	/// <summary>
	///  Enumerator for the contact collection.
	/// </summary>
	public struct Enumerator : IEnumerator<ContactInformation>, IDisposable, IEnumerator
	{
	...
	public ContactInformation this[int index]
	{
		get
		{
			pair.GetContactInformation(index, out var info);
			return info;
		}
		set
		{
	...
	public void CopyTo(ContactInformation[] array, int arrayIndex)
	{
		...
		for (int i = 0; i < count; i++)
		{
			ref ContactInformation reference = ref array[arrayIndex + i];
			reference = this[i];
		}
	}
