...
public static class MotionSettings
{
	...
	private static float coreShapeScaling;
	...
	public static float CoreShapeScaling
	{
		...
		set
		{
			coreShapeScaling = MathHelper.Clamp(value, 0f, 0.99f);
		}
	}

	internal static bool PairAllowsCCD(Entity entity, CollidablePairHandler pair)
	{
		Collidable other = ((pair.broadPhaseOverlap.entryA == entity.CollisionInformation) ? pair.broadPhaseOverlap.entryB : pair.broadPhaseOverlap.entryA) as Collidable;
		return CCDFilter(entity, other, pair);
	}

	private static bool DefaultCCDFilter(Entity entity, Collidable other, CollidablePairHandler pair)
	{
	...
	{
		coreShapeScaling = 0.8f;
		DefaultPositionUpdateMode = PositionUpdateMode.Discrete;
