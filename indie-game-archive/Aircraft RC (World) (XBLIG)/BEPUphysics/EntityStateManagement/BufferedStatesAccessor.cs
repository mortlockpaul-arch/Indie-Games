...
public class BufferedStatesAccessor
{
	...
	public Matrix3X3 OrientationMatrix
	{
		get
		{
			...
			if (IsReadBufferAccessible())
			{
				Quaternion quaternion = bufferedStates.BufferedStatesManager.ReadBuffers.GetState(bufferedStates.motionStateIndex).Orientation;
		...
		set
		{
			if (IsWriteBufferAccessible())
			{
				Quaternion newOrientationQuaternion = Quaternion.Normalize(Quaternion.CreateFromRotationMatrix(Matrix3X3.ToMatrix4X4(value)));
				WriteBuffer.EnqueueOrientation(bufferedStates.Entity, ref newOrientationQuaternion);
			}
			else
			{
	...
	public Matrix WorldTransform
	{
		...
		set
		{
			if (IsWriteBufferAccessible())
			{
				...
				result.Normalize();
				WriteBuffer.EnqueueOrientation(bufferedStates.Entity, ref result);
				WriteBuffer.EnqueuePosition(bufferedStates.Entity, ref newPosition);
			}
			else
			{
	...
	private bool IsReadBufferAccessible()
	{
		...
		{
			return bufferedStates.BufferedStatesManager.ReadBuffers.Enabled;
		}
		return false;
	}
