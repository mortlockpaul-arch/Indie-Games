...
public class CylinderShape : ConvexShape
{
	...
	public override void GetLocalExtremePointWithoutMargin(ref Vector3 direction, out Vector3 extremePoint)
	{
		...
		if (num > 1E-07f)
		{
			float num2 = (radius - collisionMargin) / (float)Math.Sqrt(num);
			extremePoint = new Vector3(direction.X * num2, (float)Math.Sign(direction.Y) * (halfHeight - collisionMargin), direction.Z * num2);
		}
		else
		{
			extremePoint = new Vector3(0f, (float)Math.Sign(direction.Y) * (halfHeight - collisionMargin), 0f);
		}
	}
