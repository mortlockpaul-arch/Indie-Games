...
public abstract class ContactManifoldConstraint : SolverGroup
{
	...
	public override void UpdateSolverActivity()
	{
		if (isActive)
		{
			...
			isActiveInSolver = pair.BroadPhaseOverlap.collisionRule < CollisionRule.NoSolver && ((entityA != null && entityA.isDynamic && entityA.activityInformation.IsActive) || (entityB != null && entityB.isDynamic && entityB.activityInformation.IsActive));
	...
	public void UpdateMaterialProperties(Material materialA, Material materialB)
	{
		...
		{
			materialInteraction.KineticFriction = 0f;
			materialInteraction.StaticFriction = 0f;
			materialInteraction.Bounciness = 0f;
		}
	}
}
