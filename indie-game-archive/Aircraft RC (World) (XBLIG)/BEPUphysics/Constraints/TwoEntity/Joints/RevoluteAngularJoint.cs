...
/// </summary>
public class RevoluteAngularJoint : Joint, I2DImpulseConstraintWithError, I2DImpulseConstraint, I2DJacobianConstraint
{
	...
	private Vector3 localConstrainedAxis1;

	private Vector3 localConstrainedAxis2;
	...
	private Vector3 worldConstrainedAxis1;

	private Vector3 worldConstrainedAxis2;
	...
	public Vector2 RelativeVelocity
	{
		get
		{
			...
			Vector3.Dot(ref worldConstrainedAxis1, ref result, out result2.X);
			Vector3.Dot(ref worldConstrainedAxis2, ref result, out result2.Y);
			return result2;
		}
	}
	...
	public override float SolveIteration()
	{
		...
		Vector3.Dot(ref worldConstrainedAxis1, ref result, out value.X);
		Vector3.Dot(ref worldConstrainedAxis2, ref result, out value.Y);
		Vector2.Add(ref value, ref biasVelocity, out value);
		Vector2.Multiply(ref accumulatedImpulse, softness, out var result2);
		Vector2.Add(ref value, ref result2, out value);
		Matrix2X2.Transform(ref value, ref effectiveMassMatrix, out value);
		Vector2.Add(ref accumulatedImpulse, ref value, out accumulatedImpulse);
		Vector3 impulse = new Vector3
		{
			X = worldConstrainedAxis1.X * value.X + worldConstrainedAxis2.X * value.Y,
			Y = worldConstrainedAxis1.Y * value.X + worldConstrainedAxis2.Y * value.Y,
			Z = worldConstrainedAxis1.Z * value.X + worldConstrainedAxis2.Z * value.Y
		};
	...
	public override void Update(float dt)
	{
		...
		Vector3.Cross(ref worldAxisA, ref worldAxisB, out var result);
		Vector3.Dot(ref result, ref worldConstrainedAxis1, out error.X);
		Vector3.Dot(ref result, ref worldConstrainedAxis2, out error.Y);
		...
		{
			Matrix3X3.Add(ref connectionA.inertiaTensorInverse, ref connectionB.inertiaTensorInverse, out var result2);
			Matrix3X3.Transform(ref worldConstrainedAxis1, ref result2, out result3);
			Matrix3X3.Transform(ref worldConstrainedAxis2, ref result2, out result4);
		}
		else if (connectionA.isDynamic && !connectionB.isDynamic)
		{
			Matrix3X3.Transform(ref worldConstrainedAxis1, ref connectionA.inertiaTensorInverse, out result3);
			Matrix3X3.Transform(ref worldConstrainedAxis2, ref connectionA.inertiaTensorInverse, out result4);
		}
		else
		{
			if (connectionA.isDynamic || !connectionB.isDynamic)
			...
			Matrix3X3.Transform(ref worldConstrainedAxis1, ref connectionB.inertiaTensorInverse, out result3);
			Matrix3X3.Transform(ref worldConstrainedAxis2, ref connectionB.inertiaTensorInverse, out result4);
		}
		Vector3.Dot(ref result3, ref worldConstrainedAxis1, out effectiveMassMatrix.M11);
		Vector3.Dot(ref result3, ref worldConstrainedAxis2, out effectiveMassMatrix.M12);
		Vector3.Dot(ref result4, ref worldConstrainedAxis1, out effectiveMassMatrix.M21);
		Vector3.Dot(ref result4, ref worldConstrainedAxis2, out effectiveMassMatrix.M22);
	...
	public override void ExclusiveUpdate()
	{
		Vector3 impulse = new Vector3
		{
			X = worldConstrainedAxis1.X * accumulatedImpulse.X + worldConstrainedAxis2.X * accumulatedImpulse.Y,
			Y = worldConstrainedAxis1.Y * accumulatedImpulse.X + worldConstrainedAxis2.Y * accumulatedImpulse.Y,
			Z = worldConstrainedAxis1.Z * accumulatedImpulse.X + worldConstrainedAxis2.Z * accumulatedImpulse.Y
		};
		if (connectionA.isDynamic)
		{
			connectionA.ApplyAngularImpulse(ref impulse);
	...
	private void UpdateRestrictedAxes()
	{
		...
		if (localConstrainedAxis1.LengthSquared() < 0.001f)
		{
