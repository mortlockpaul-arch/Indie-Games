...
/// </summary>
public class EntityRotator : Updateable, IDuringForcesUpdateable, ISpaceUpdateable, ISpaceObject
{
	...
	{
		Quaternion.Conjugate(ref start, out var result);
		Quaternion.Multiply(ref end, ref result, out result);
		Toolbox.GetAxisAngleFromQuaternion(ref result, out var axis, out var angle);
	...
	void IDuringForcesUpdateable.Update(float dt)
	{
		...
		{
			throw new InvalidOperationException("EntityRotator's entity differs from EntityRotator's motor's entities.  Ensure that the moved entity is only changed by setting the EntityRotator's entity property.");
		}
		if (Entity.IsDynamic)
		{
