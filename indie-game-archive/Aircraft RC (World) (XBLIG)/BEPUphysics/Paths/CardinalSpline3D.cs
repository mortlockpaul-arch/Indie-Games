...
public class CardinalSpline3D : HermiteCurve3D
{
	...
	public float Tension
	{
		...
		set
		{
			tension = MathHelper.Clamp(value, 0f, 1f);
		}
	}
	...
	{
		if (base.ControlPoints.Count > 3)
		{
			minIndex = 1;
			maxIndex = base.ControlPoints.Count - 2;
		}
		else
		{
			minIndex = -1;
	...
	protected override void ComputeTangents()
	{
		...
		{
			Vector3 value = base.ControlPoints[i - 1].Value;
			Vector3 value2 = base.ControlPoints[i + 1].Value;
			Vector3.Subtract(ref value2, ref value, out var result);
			Vector3.Multiply(ref result, (1f - tension) / (base.ControlPoints[i + 1].Time - base.ControlPoints[i - 1].Time), out result);
			tangents.Add(result);
		}
		tangents.Add(Vector3.Zero);
	}
