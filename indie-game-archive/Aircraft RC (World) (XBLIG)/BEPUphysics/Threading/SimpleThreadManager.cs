...
/// </remarks>
public class SimpleThreadManager : IThreadManager, IDisposable
{
	...
	private class WorkerThread : IDisposable
	{
		...
		{
			if (Interlocked.Decrement(ref manager.tasksRemaining) == 0 && !manager.disposed)
			{
			...
			while (true)
			{
				...
				lock (taskQueue)
				{
					if (taskQueue.Count > 0)
					{
						...
						obj = taskInformationQueue.Dequeue();
					}
				}
				if (action != null)
				{
	...
	private readonly ManualResetEvent allThreadsIdleNotifier = new ManualResetEvent(initialState: false);
	...
	public SimpleThreadManager()
	{
		...
		doLoopSectionDelegate = DoLoopSection;
	}
	...
	public void AddThread(Action<object> initialization, object initializationInformation)
	{
		lock (workers)
		{
			WorkerThread item = new WorkerThread(this, initialization, initializationInformation);
			workers.Add(item);
			RemakeLoopSections();
		}
	}
	...
	{
		int num = workers.Count * loopTasksPerThread;
		int iterationCount = endIndex - startIndex;
		for (int i = 0; i < num; i++)
		{
			taskInfos[i].loopBody = loopBody;
			taskInfos[i].iterationCount = iterationCount;
			EnqueueTaskSequentially(doLoopSectionDelegate, taskInfos[i]);
		}
		WaitForTaskCompletion();
	}
	...
	public void ShutDown()
	{
		...
		for (int num = workers.Count - 1; num >= 0; num--)
		{
			lock (workers)
			{
				queue.Enqueue(workers[num].Thread);
				workers[num].EnqueueTask(null, null);
			}
		}
		while (queue.Count > 0)
		{
	...
	{
		LoopSection loopSection = o as LoopSection;
		int num = loopSection.iterationCount * (loopSection.Index + 1) / loopSection.Subdivisions;
		for (int i = loopSection.iterationCount * loopSection.Index / loopSection.Subdivisions; i < num; i++)
		{
			loopSection.loopBody(i);
		}
	}
	...
	private void RemakeLoopSections()
	{
		...
		int subdivisions = count * loopTasksPerThread;
