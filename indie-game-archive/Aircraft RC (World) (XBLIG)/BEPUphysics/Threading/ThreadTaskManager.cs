...
/// </summary>
public class ThreadTaskManager : IThreadManager, IDisposable
{
	...
	private class WorkerThread : IDisposable
	{
		...
		private void ThreadExecutionLoop()
		{
			...
			while (true)
			{
				...
				while (true)
				{
					if (!taskData.TryDequeueFirst(out var item))
					{
						bool flag = false;
						for (int i = 1; i < manager.workers.Count; i++)
						{
							if (TrySteal((index + i) % manager.workers.Count, out item))
							{
								flag = true;
								break;
							}
	...
	private ManualResetEvent allThreadsIdleNotifier = new ManualResetEvent(initialState: false);
	...
	public void AddThread(Action<object> initialization, object initializationInformation)
	{
		lock (workers)
		{
			WorkerThread item = new WorkerThread(workers.Count, this, initialization, initializationInformation);
			workers.Add(item);
			RemakeLoopSections();
		}
	}
