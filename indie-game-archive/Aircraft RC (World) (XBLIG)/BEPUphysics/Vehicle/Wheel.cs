...
public class Wheel
{
	...
	internal Vector3 localForwardDirection = new Vector3(0f, 0f, -1f);
	...
	public Vector3 WorldForwardDirection
	{
		...
		set
		{
			...
			if (vehicle != null)
			{
				Quaternion.Conjugate(ref Vehicle.Body.orientation, out var result);
				Vector3.Transform(ref worldForwardDirection, ref result, out localForwardDirection);
			}
			else
			{
	...
	internal void PreStep(float dt)
	{
		...
		else
		{
			...
			suspension.accumulatedImpulse = 0f;
			slidingFriction.accumulatedImpulse = 0f;
			drivingMotor.accumulatedImpulse = 0f;
			brake.accumulatedImpulse = 0f;
		}
	}
	...
	internal void OnAddedToVehicle(Vehicle vehicle)
	{
		...
		ISpace space = ((ISpaceObject)vehicle).Space;
		if (space != null)
		{
