...
{
	private float airborneWheelAcceleration = 40f;

	private float airborneWheelDeceleration = 4f;

	private float brakeFreezeWheelDeceleration = 40f;

	/// <summary>
	/// Collects collision pairs from the environment.
	/// </summary>
	protected internal Box detector = new Box(Vector3.Zero, 0f, 0f, 0f);
	...
	public float AirborneWheelAcceleration
	{
		get
		{
			return airborneWheelAcceleration;
		}
		set
		{
			airborneWheelAcceleration = Math.Abs(value);
		}
	}
	...
	public float AirborneWheelDeceleration
	{
		get
		{
			return airborneWheelDeceleration;
		}
		set
		{
			airborneWheelDeceleration = Math.Abs(value);
		}
	}
	...
	public float BrakeFreezeWheelDeceleration
	{
		get
		{
			return brakeFreezeWheelDeceleration;
		}
		set
		{
			brakeFreezeWheelDeceleration = Math.Abs(value);
		}
	}
	...
	public Matrix LocalGraphicTransform
	{
		get
		{
			return localGraphicTransform;
		}
		set
		{
			localGraphicTransform = value;
		}
	}
	...
	internal void UpdateSpin(float dt)
	{
		...
		else if (wheel.HasSupport && wheel.brake.IsBraking && FreezeWheelsWhileBraking)
		{
			...
			if (spinVelocity > 0f)
			{
				value = Math.Max(spinVelocity - brakeFreezeWheelDeceleration * dt, 0f);
			}
			else if (spinVelocity < 0f)
			{
				value = Math.Min(spinVelocity + brakeFreezeWheelDeceleration * dt, 0f);
			}
			spinVelocity = wheel.drivingMotor.RelativeVelocity / Radius;
			if (Math.Abs(value) < Math.Abs(spinVelocity))
			{
				spinVelocity = value;
			}
		}
		else if (!wheel.HasSupport && wheel.drivingMotor.TargetSpeed != 0f)
		{
			float num = Math.Abs(wheel.drivingMotor.TargetSpeed) / Radius;
			spinVelocity = MathHelper.Clamp(spinVelocity + (float)Math.Sign(wheel.drivingMotor.TargetSpeed) * airborneWheelAcceleration * dt, 0f - num, num);
		}
		else if (!wheel.HasSupport && wheel.Brake.IsBraking)
		{
			if (spinVelocity > 0f)
			{
				spinVelocity = Math.Max(spinVelocity - brakeFreezeWheelDeceleration * dt, 0f);
			}
			else if (spinVelocity < 0f)
			{
				spinVelocity = Math.Min(spinVelocity + brakeFreezeWheelDeceleration * dt, 0f);
			}
		}
		else if (!wheel.HasSupport)
		{
			if (spinVelocity > 0f)
			{
				spinVelocity = Math.Max(spinVelocity - airborneWheelDeceleration * dt, 0f);
			}
			else if (spinVelocity < 0f)
			{
				spinVelocity = Math.Min(spinVelocity + airborneWheelDeceleration * dt, 0f);
			}
		}
