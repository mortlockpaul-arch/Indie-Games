...
public abstract class StaticMeshContactManifold : TriangleMeshConvexContactManifold
{
	...
	protected override bool ConfigureTriangle(int i, out TriangleIndices indices)
	{
		...
		mesh.Mesh.Data.GetTriangle(num, out localTriangleShape.vA, out localTriangleShape.vB, out localTriangleShape.vC);
		localTriangleShape.sidedness = mesh.sidedness;
		localTriangleShape.collisionMargin = 0f;
		indices = new TriangleIndices
		{
			A = mesh.Mesh.Data.indices[num],
			B = mesh.Mesh.Data.indices[num + 1],
			C = mesh.Mesh.Data.indices[num + 2]
		};
		return true;
	}
