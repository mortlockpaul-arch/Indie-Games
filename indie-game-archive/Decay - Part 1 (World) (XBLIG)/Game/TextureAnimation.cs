...
public class TextureAnimation : Animation2D
{
	...
	public override void Clear()
	{
		...
		if (m_frames != null)
		{
			...
			{
				((GraphicsResource)m_frames[i]).Dispose();
				m_frames[i] = null;
			}
			m_frames.Clear();
			m_frames = null;
	...
	public void UseCombinedFrames(int frame_width, int frame_height, int total_frames)
	{
		m_frame_width = frame_width;
		m_frame_height = frame_height;
		m_frames_x = (int)Math.Floor(2048f / (float)frame_width);
		m_frames_y = (int)Math.Floor(2048f / (float)frame_height);
	...
	public override void Draw(SpriteBatch SB, Color c)
	{
		...
		m_current_alpha = (int)((Color)(ref c)).A;
		if (m_update_chain_animation)
		{
		...
		((Rectangle)(ref value))._002Ector(0, 0, m_frame_width, m_frame_height);
		value.X = m_local_frame_x * m_frame_width;
		value.Y = m_local_frame_y * m_frame_height;
		if (m_texture != null)
		{
			SB.Begin((SpriteBlendMode)1);
			SB.Draw(m_texture, Game.VIEW_RECT, (Rectangle?)value, c);
			SB.End();
		}
		if (m_frame_smoothing && ((Color)(ref c)).A == byte.MaxValue)
		{
			...
			if (m_smoothing_texture != null)
			{
				...
				SB.Begin((SpriteBlendMode)1);
				SB.Draw(m_smoothing_texture, Game.VIEW_RECT, (Rectangle?)value, new Color(byte.MaxValue, byte.MaxValue, byte.MaxValue, (byte)m_smoothing_alpha));
				SB.End();
			}
		}
	}
	...
	public override void Draw(SpriteBatch SB, Vector2 pos, Color color)
	{
		...
		((Animation2D)this).Draw(SB, new Rectangle((int)pos.X, (int)pos.Y, m_width, m_height), new Rectangle(0, 0, m_width, m_height), color);
	}
	...
	public override void Draw(SpriteBatch SB, Rectangle dest_rect, Rectangle source_rect, Color color)
	{
		...
		try
		{
			m_current_alpha = (int)((Color)(ref color)).A;
			if (m_update_chain_animation)
			{
			...
			if (m_combined_frames == 0)
			{
				...
				if (m_frame_smoothing)
				{
					m_smoothing_texture = m_frames[m_smoothing_frame];
					SB.Begin((SpriteBlendMode)1);
					SB.Draw(m_smoothing_texture, dest_rect, (Rectangle?)source_rect, new Color(byte.MaxValue, byte.MaxValue, byte.MaxValue, (byte)m_smoothing_alpha));
					SB.End();
				}
				return;
			}
			...
			{
				SB.Begin((SpriteBlendMode)1);
				SB.Draw(m_texture, dest_rect, (Rectangle?)value, color);
				SB.End();
			}
			if (m_frame_smoothing && ((Color)(ref color)).A == byte.MaxValue)
			{
				index = (m_reverse ? ((int)Math.Floor((float)(m_current_frame + 1) / (float)m_combined_frames)) : ((int)Math.Floor((float)(m_current_frame - 1) / (float)m_combined_frames)));
				if (index >= 0 && index < m_frames.Count)
				{
				...
				if (m_smoothing_texture != null)
				{
					...
					value.Y = m_prev_local_frame_y * m_frame_height;
					SB.Begin((SpriteBlendMode)1);
					SB.Draw(m_smoothing_texture, dest_rect, (Rectangle?)value, new Color(byte.MaxValue, byte.MaxValue, byte.MaxValue, (byte)m_smoothing_alpha));
					SB.End();
				}
			}
		}
