...
internal abstract class Player
{
	...
	private Color m_NewLightColour;

	private Color m_StartingLightColour;
	...
	public Player(byte playerid, Vector3 pos, Gamer gamer, ShipColor colour, ETeam team)
	{
		...
		IsActive = true;
		LightColour = new Color((byte)0, (byte)0, (byte)0, byte.MaxValue);
		m_NewLightColour = new Color((byte)0, (byte)0, (byte)0, byte.MaxValue);
		m_StartingLightColour = new Color((byte)0, (byte)0, (byte)0, byte.MaxValue);
		base._002Ector();
	...
	protected virtual void Reset(bool newGame)
	{
		...
		((Color)(ref LightColour)).PackedValue = 4278190080u;
		((Color)(ref m_NewLightColour)).PackedValue = 4278190080u;
	...
	public virtual void Update()
	{
		...
		if (m_ChangingColour)
		{
			...
			if (num >= 1.0)
			{
				((Color)(ref LightColour)).PackedValue = ((Color)(ref m_NewLightColour)).PackedValue;
				m_ChangingColour = false;
				m_ChangingColourStartTime = 0.0;
			}
			else
			{
				uint num2 = (uint)((double)(((Color)(ref m_NewLightColour)).R - ((Color)(ref m_StartingLightColour)).R) * num + (double)(int)((Color)(ref m_StartingLightColour)).R);
				uint num3 = (uint)((double)(((Color)(ref m_NewLightColour)).G - ((Color)(ref m_StartingLightColour)).G) * num + (double)(int)((Color)(ref m_StartingLightColour)).G);
				uint num4 = (uint)((double)(((Color)(ref m_NewLightColour)).B - ((Color)(ref m_StartingLightColour)).B) * num + (double)(int)((Color)(ref m_StartingLightColour)).B);
				uint num5 = (uint)((double)(((Color)(ref m_NewLightColour)).A - ((Color)(ref m_StartingLightColour)).A) * num + (double)(int)((Color)(ref m_StartingLightColour)).A);
				((Color)(ref LightColour)).PackedValue = (num5 << 24) + (num2 << 16) + (num3 << 8) + num4;
			}
		}
	}
	...
	public virtual void Respawn(RespawnLocation pos)
	{
		...
		Reset(newGame: false);
		IsActive = true;
		LightColour = new Color((byte)0, (byte)0, byte.MaxValue, byte.MaxValue);
		MakeInvincible(5f);
	}
	...
	private void ChangeColour()
	{
		...
		((Color)(ref m_NewLightColour)).PackedValue = (num4 << 24) + (num << 16) + (num2 << 8) + num3;
		((Color)(ref m_StartingLightColour)).PackedValue = ((Color)(ref LightColour)).PackedValue;
