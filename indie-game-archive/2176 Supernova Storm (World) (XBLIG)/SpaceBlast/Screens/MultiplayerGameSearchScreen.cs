...
internal class MultiplayerGameSearchScreen : MenuScreen
{
	...
	private void GameSearchCompleteEvent(object sender, EventArgs e)
	{
		...
		AvailableNetworkSession val = null;
		int num = 0;
		foreach (AvailableNetworkSession item in (ReadOnlyCollection<AvailableNetworkSession>)(object)gameSearchList)
		{
		...
		if (val != null)
		{
			if (!MainGame.NetMan.JoinGameSession(val))
			{
	...
	private void RecalcScreenRect()
	{
		...
		((Vector2)(ref val))._002Ector(450f, 175f);
