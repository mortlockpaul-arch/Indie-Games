...
internal class AudioManager
{
	...
	private SoundEffect[] m_Sounds = new SoundEffect[13];

	private SoundEffectInstance[] m_SoundInstances = new SoundEffectInstance[100];
	...
	{
		SoundEffectInstance soundEffectInstance = m_Sounds[4].Play(1f, 0f, 0f, true);
	...
	{
		Vector3 vector = position - MainGame.Instance.LeftPlayer.TheShip.Position;
		float num = MathHelper.Clamp(1f - vector.LengthSquared() / 7E+09f, 0f, 1f);
		float num2 = vector.X / 20000f;
		...
		{
			vector = position - MainGame.Instance.RightPlayer.TheShip.Position;
			num = MathHelper.Clamp(1f - vector.LengthSquared() / 7E+09f, 0f, 1f);
			num2 = vector.X / 20000f;
	...
	public void PlayEngines(Player player, float speed, float thrust, ref Vector3 position)
	{
		try
		{
			float value = MathHelper.Clamp(1f - (position - MainGame.Instance.LeftPlayer.TheShip.Position).LengthSquared() / 7E+09f, 0f, 1f);
			float value2 = 0f;
			if (MainGame.Instance.RightPlayer != null)
			{
				value2 = MathHelper.Clamp(1f - (position - MainGame.Instance.RightPlayer.TheShip.Position).LengthSquared() / 7E+09f, 0f, 1f);
			}
			if (player.EngineSound.State != SoundState.Playing)
			{
				player.EngineSound.Resume();
			}
			player.EngineSound.Volume = thrust * MathHelper.Max(value, value2);
