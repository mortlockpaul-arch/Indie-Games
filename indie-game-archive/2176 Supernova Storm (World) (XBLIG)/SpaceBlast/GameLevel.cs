...
internal class GameLevel
{
	...
	public void LoadLevel(int levelnumber)
	{
		...
		foreach (XmlNode item2 in xmlNodeList)
		{
			...
			float rotation = MathHelper.ToRadians(num);
		...
		{
			m_ContentManager = new ContentManager(MainGame.Instance.Services, "Content");
		}
		else
		{
	...
	public RespawnLocation GetPlayerRespawnPosition(byte playerid)
	{
		...
		while (num++ < 8 && !flag)
		{
			...
			BoundingSphere sphere = new BoundingSphere(result.Position, 20000f);
			flag = true;
			foreach (Player item in players)
			{
				if (item != player && item.TheShip.GetBoundingSphere().Intersects(sphere))
				{
					flag = false;
					break;
				}
