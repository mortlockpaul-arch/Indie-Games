	...
	private PowerUp ChoosePowerup()
	{
		...
		foreach (PowerUp item in powerups)
		{
			...
			case PowerUpType.FrontAmmo:
				num3 = (int)MathHelper.Clamp((float)(200 - m_Brain.Player.TheShip.Weapons.ActiveFrontWeapon.Ammo), 0f, 100f);
				break;
			...
			if (num3 > num)
			{
				result = item;
				num = num3;
				Vector3 val = theShip.Position - item.Position;
				num2 = ((Vector3)(ref val)).Length();
			}
			else if ((float)num3 == num2)
			{
				Vector3 val2 = theShip.Position - item.Position;
				float num4 = ((Vector3)(ref val2)).Length();
				if (num4 < num2)
				{
					result = item;
					num = num3;
