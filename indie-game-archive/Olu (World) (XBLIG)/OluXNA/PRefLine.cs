...
internal class PRefLine : IPath
{
	...
	public PRefLine(Vector3 _start, Vector3 _end, float _sp, Vector3 _refPoint)
	{
	...
	public PRefLine(Vector3 _start, Vector3 _end, float _sp, Vector3 _refPoint, bool _loop)
	{
	...
	public PRefLine(PRefLine other)
	{
		//IL_0008: Unknown result type (might be due to invalid IL or missing references)
	...
	public void SetEnd(Vector3 _end)
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		//IL_0002: Unknown result type (might be due to invalid IL or missing references)
		end = _end;
	...
	public PRefLine(Dictionary<string, string> attributes, XmlNode node)
	{
		//IL_000c: Unknown result type (might be due to invalid IL or missing references)
		...
		if (!attributes.ContainsKey("track") || attributes["track"] == "camera")
		{
			if (attributes.ContainsKey("loop"))
			{
				Initialize(vectorFromAtt, vectorFromAtt2, LevelLoader.GetFloatFromAtt(attributes, "speed", 0f), BaseGame.Get().playerPos, !(attributes["loop"] == "false"));
			}
			else
			{
				Initialize(vectorFromAtt, vectorFromAtt2, LevelLoader.GetFloatFromAtt(attributes, "speed", 0f), BaseGame.Get().playerPos, _loop: false);
			}
		}
	}
