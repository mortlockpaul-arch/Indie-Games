...
public class ZXGame : Game
{
	...
	public ZXGame(GameFile gameFile)
	{
	...
	protected override void Update(GameTime gameTime)
	{
		...
		{
			InfoPage infoPage = CurrentGame.InfoPages.SingleOrDefault((InfoPage p) => p.Name == "help");
			if (infoPage != null)
			{
				MenuNavigation(ref HelpPosition, infoPage.NumberOfPages);
			}
			break;
		}
		case ScreenEnum.History:
		{
			InfoPage infoPage2 = CurrentGame.InfoPages.SingleOrDefault((InfoPage p) => p.Name == "history");
			if (infoPage2 != null)
			{
				MenuNavigation(ref HistoryPosition, infoPage2.NumberOfPages);
			}
			break;
		}
		...
		case ScreenEnum.GamePlay:
			...
			if (emulate && !Guide.IsVisible)
			{
				...
				if (state.Buttons.Y == ButtonState.Pressed && !ButtonY && CurrentGame.ButtonY != null)
				{
					ButtonY = true;
	...
	protected override void Draw(GameTime gameTime)
	{
		...
		case ScreenEnum.GameTypeSelect:
		{
			...
			{
				spriteBatch.DrawString(MenuSprite, gameVersion.Name, new Vector2(400f, num5), (GameTypeSelectMenuItem == num7++) ? Color.Red : Color.White);
				num5 += num6;
			}
			break;
		}
		...
		case ScreenEnum.Settings:
		{
			int num9 = 247;
			DrawHeader();
			spriteBatch.DrawString(MenuSprite, "PIXEL FILTERING", new Vector2(num9, 300f), (SettingsItem == 0) ? Color.Red : Color.White);
			spriteBatch.DrawString(MenuSprite, Settings.PixelFiltering ? "ON" : "OFF", new Vector2(650f, 300f), Color.White);
			spriteBatch.DrawString(MenuSprite, "SCAN LINES", new Vector2(num9, 350f), (SettingsItem == 1) ? Color.Red : Color.White);
			spriteBatch.DrawString(MenuSprite, Settings.ScanLines ? "ON" : "OFF", new Vector2(650f, 350f), Color.White);
			spriteBatch.DrawString(MenuSprite, "WINTER MODE", new Vector2(num9, 400f), (SettingsItem == 2) ? Color.Red : Color.White);
			spriteBatch.DrawString(MenuSprite, Settings.WinterMode ? "ON" : "OFF", new Vector2(650f, 400f), Color.White);
			spriteBatch.DrawString(MenuSprite, "FULL SCREEN", new Vector2(num9, 450f), (SettingsItem == 3) ? Color.Red : Color.White);
		...
		case ScreenEnum.About:
		{
			...
			int num3 = 30;
			int num4 = 50;
			spriteBatch.DrawCenteredString(TextSprite, CurrentGame.CopyrightText, new Vector2(num2, num), 930f);
			num += num3;
