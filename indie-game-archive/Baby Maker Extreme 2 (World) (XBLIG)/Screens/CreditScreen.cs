...
public class CreditScreen : Screen
{
	...
	{
		m_bg = TextureContainer.GetSprite("images/whitesquare", default(Vector2), DepthConsts.FADE_DEPTH);
	...
	public override void Draw(TimeTracker gameTime)
	{
		...
		{
			SceneRenderer.DrawStringCentered(fonts.ITEM_COUNT_FONT, m_textBase[i], SceneRenderer.GetCameraPosition() + new Vector2(0f, 0f - m_fOffset + num), white, m_bg.Depth + 1f);
			num += 70f;
		}
		num += 40f;
		for (int j = 0; j < m_textMusic.Count; j++)
		{
			SceneRenderer.DrawStringCentered(fonts.CASH_COUNT_FONT, m_textMusic[j], SceneRenderer.GetCameraPosition() + new Vector2(0f, 0f - m_fOffset + num), white, m_bg.Depth + 1f);
			num += 40f;
		}
		num += 40f;
		for (int k = 0; k < m_textFreeSound.Count; k++)
		{
			SceneRenderer.DrawStringCentered(fonts.CASH_COUNT_FONT, m_textFreeSound[k], SceneRenderer.GetCameraPosition() + new Vector2(0f, 0f - m_fOffset + num), white, m_bg.Depth + 1f);
			num += 40f;
		}
		m_fExitPos = num + 720f;
	}
	...
	{
		if (!m_bExit && (ControlManager.PressedBackButton(ControlManager.ActiveMenuIndex) || ControlManager.PressedButton(ControlManager.ActiveMenuIndex, Buttons.B)))
		{
			m_bExit = true;
			m_fFadeTimer = 1f;
		}
