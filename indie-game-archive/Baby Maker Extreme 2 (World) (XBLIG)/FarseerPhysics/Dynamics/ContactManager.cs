...
public class ContactManager
{
	...
	private static bool ShouldCollide(Fixture fixtureA, Fixture fixtureB)
	{
		...
		{
			if (fixtureA.CollisionFilter.CollisionGroup == fixtureB.CollisionFilter.CollisionGroup && fixtureA.CollisionFilter.CollisionGroup != 0 && fixtureB.CollisionFilter.CollisionGroup != 0)
			{
				return false;
			}
			if (((fixtureA.CollisionFilter.CollisionCategories & fixtureB.CollisionFilter.CollidesWith) == 0) & ((fixtureB.CollisionFilter.CollisionCategories & fixtureA.CollisionFilter.CollidesWith) == 0))
			{
				return false;
			}
		...
		bool flag = (fixtureA.CollisionFilter.CollidesWith & fixtureB.CollisionFilter.CollisionCategories) != Category.None && (fixtureA.CollisionFilter.CollisionCategories & fixtureB.CollisionFilter.CollidesWith) != 0;
		if (flag && (fixtureA.CollisionFilter.IsFixtureIgnored(fixtureB) || fixtureB.CollisionFilter.IsFixtureIgnored(fixtureA)))
		{
			return false;
		}
		return flag;
