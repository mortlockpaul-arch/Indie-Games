...
public class World
{
	...
	private FarseerPhysics.Common.HashSet<Body> _bodyAddList = new FarseerPhysics.Common.HashSet<Body>(32);

	private FarseerPhysics.Common.HashSet<Body> _bodyRemoveList = new FarseerPhysics.Common.HashSet<Body>(32);

	private FarseerPhysics.Common.HashSet<Joint> _jointAddList = new FarseerPhysics.Common.HashSet<Joint>(32);

	private FarseerPhysics.Common.HashSet<Joint> _jointRemoveList = new FarseerPhysics.Common.HashSet<Joint>(32);
	...
	{
		FixtureProxy userData = ContactManager.BroadPhase.GetUserData(proxyId);
		return _queryAABBCallback(userData.Fixture);
	}
	...
	{
		Island.Reset(BodyList.Count, ContactManager.ContactCount, JointList.Count, ContactManager);
	...
	private void SolveTOI(ref TimeStep step)
	{
		...
		while (true)
		{
			...
			float num = 1f;
			for (Contact contact3 = ContactManager.ContactList; contact3 != null; contact3 = contact3.Next)
			{
				...
				else
				{
					...
					bool flag3 = body.IsBullet || bodyType != BodyType.Dynamic;
					bool flag4 = body2.IsBullet || bodyType2 != BodyType.Dynamic;
					if (!flag3 && !flag4)
					{
						continue;
					}
					...
					input.ProxyA.Set(fixtureA.Shape, contact3.ChildIndexA);
					input.ProxyB.Set(fixtureB.Shape, contact3.ChildIndexB);
			...
			for (int k = 0; k < Island.BodyCount; k++)
			{
				Body body6 = Island.Bodies[k];
