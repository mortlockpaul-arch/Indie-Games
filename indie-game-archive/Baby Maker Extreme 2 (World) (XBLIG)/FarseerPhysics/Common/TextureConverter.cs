...
public static class TextureConverter
{
	private sealed class CrossingEdgeInfo : IComparable
	{
		...
		public int CompareTo(object obj)
		{
			...
			case EdgeAlignment.Vertical:
				if (CrossingPoint.X < crossingEdgeInfo.CrossingPoint.X)
				{
					result = -1;
				}
				else if (CrossingPoint.X > crossingEdgeInfo.CrossingPoint.X)
				{
					result = 1;
				}
				break;
			case EdgeAlignment.Horizontal:
				if (CrossingPoint.Y < crossingEdgeInfo.CrossingPoint.Y)
				{
					result = -1;
				}
				else if (CrossingPoint.Y > crossingEdgeInfo.CrossingPoint.Y)
				{
					result = 1;
				}
				break;
	...
	public static Vertices DetectVertices(uint[] data, int width, int height)
	{
		...
		List<Vertices> list = DetectVertices(ref pca);
		return list[0];
	}
	...
	public static List<Vertices> DetectVertices(uint[] data, int width, int height, float hullTolerance, byte alphaTolerance, bool multiPartDetection, bool holeDetection)
	{
		PolygonCreationAssistance pca = new PolygonCreationAssistance(data, width, height)
		{
	...
	private static List<Vertices> DetectVertices(ref PolygonCreationAssistance pca)
	{
		...
		do
		{
			...
			while (GetNextHullEntrance(pca, entrance.Value, out entrance))
			{
				...
				{
					vertices = list[i];
					if (InPolygon(pca, vertices, entrance.Value))
					{
						flag2 = true;
						break;
					}
	...
	private static Vector2? GetHoleHullEntrance(PolygonCreationAssistance pca, Vertices polygon, Vector2? startVertex)
	{
		...
		if (polygon != null && polygon.Count > 0)
		{
			...
			if (num2 > 0 && num2 < pca.Height && num3 > 0 && num3 < pca.Height)
			{
				...
				{
					List<CrossingEdgeInfo> crossingEdges = GetCrossingEdges(polygon, EdgeAlignment.Vertical, i);
					if (crossingEdges.Count <= 1 || crossingEdges.Count % 2 != 0)
					{
						continue;
					}
					for (int j = 0; j < crossingEdges.Count; j += 2)
					{
						bool flag = false;
						bool flag2 = false;
						for (int k = (int)crossingEdges[j].CrossingPoint.X; k <= (int)crossingEdges[j + 1].CrossingPoint.X; k++)
						{
							if (pca.IsSolid(k, i))
							{
								if (!flag2)
			...
			case EdgeAlignment.Vertical:
			{
				for (int i = 0; i < polygon.Count; i++)
				{
					...
					if (((vector2.Y >= (float)checkLine && vector.Y <= (float)checkLine) || (vector2.Y <= (float)checkLine && vector.Y >= (float)checkLine)) && vector2.Y != vector.Y)
					{
						...
						if (flag)
						{
							Vector2 crossingPoint = new Vector2(((float)checkLine - vector2.Y) / vector3.Y * vector3.X + vector2.X, checkLine);
							list.Add(new CrossingEdgeInfo(crossingPoint, edgeAlign));
						}
					}
					vector = vector2;
				}
	...
	private static Vertices CreateSimplePolygon(PolygonCreationAssistance pca, Vector2 entrance, Vector2 last)
	{
		...
		else if (pca.IsSolid(ref entrance))
		{
			Vector2 foundPixel;
			...
			else if (SearchNearPixels(pca, searchingForSolidPixel: false, ref entrance, out foundPixel))
			{
				current = foundPixel;
				flag = true;
			}
		...
		for (int i = (int)start.X + (int)start.Y * pca.Width; i <= num; i++)
		{
			if (pca.IsSolid(i))
			{
				if (flag)
				{
					...
					entrance = new Vector2(num2, (float)(i - num2) / (float)pca.Width);
					return true;
				}
			}
			else
