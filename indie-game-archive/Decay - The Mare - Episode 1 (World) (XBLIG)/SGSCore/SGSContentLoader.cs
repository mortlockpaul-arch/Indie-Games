...
public class SGSContentLoader
{
	...
	public SGSContentLoader(IServiceProvider SP)
	{
		...
		m_CM.RootDirectory = CONTENT_PATH;
	...
	public virtual SGSModelContent LoadModel(string path)
	{
		if (m_CM == null)
		{
			return null;
		...
		SGSContent content = GetContent(path);
		SGSModelContent sGSModelContent = null;
		if (content == null)
		{
			sGSModelContent = new SGSModelContent(path);
			sGSModelContent.Load(m_CM, path);
			AddContent(sGSModelContent);
		}
		else
		{
			sGSModelContent = (SGSModelContent)content;
		}
		return sGSModelContent;
	}
