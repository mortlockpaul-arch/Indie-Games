...
public class CEventProgram
{
	...
	public virtual CObject evt_FirstObjectFromType(short nType)
	{
		...
		{
			return evt_SelectAllFromType(nType, bStore: true);
		}
		int num = 0;
		do
		{
	...
	public void checkEndOfPauseXBOX()
	{
		...
		bool flag = false;
		flag |= state.Buttons.A == ButtonState.Pressed;
		flag |= state.Buttons.B == ButtonState.Pressed;
		flag |= state.Buttons.X == ButtonState.Pressed;
		flag |= state.Buttons.Y == ButtonState.Pressed;
		flag |= state.Buttons.Back == ButtonState.Pressed;
		flag |= state.Buttons.LeftShoulder == ButtonState.Pressed;
		flag |= state.Buttons.RightShoulder == ButtonState.Pressed;
		flag |= state.Buttons.LeftStick == ButtonState.Pressed;
		flag |= state.Buttons.RightStick == ButtonState.Pressed;
		flag |= state.Buttons.Start == ButtonState.Pressed;
		flag |= state.DPad.Up == ButtonState.Pressed;
		flag |= state.DPad.Down == ButtonState.Pressed;
		flag |= state.DPad.Left == ButtonState.Pressed;
		if (flag | (state.DPad.Right == ButtonState.Pressed))
		{
			rhPtr.resume();
